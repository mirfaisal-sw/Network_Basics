...
static int exynos_v920_pcie_wr_other_conf(struct pcie_port *pp,
		struct pci_bus *bus, u32 devfn, int where, int size, u32 val);
static void exynos_v920_pcie_prog_viewport_cfg(struct pcie_port *pp, u32 busdev, u32 type);
...
static void exynos_v920_pcie_send_evt(struct exynos_pcie *exynos_pcie,
	u16 evt);
...
static int sriov_create_vf_v920(struct exynos_pcie *exynos_pcie, int num_vfs)
{
	...
	if (exynos_pcie->state == STATE_LINK_UP) {
		...
		if (exynos_pcie->use_sysmmu) {
			...
			iommu_map(domain, GIC_TRANSLATER_ADDR, GIC_TRANSLATER_ADDR,
					SZ_4K, IOMMU_READ | IOMMU_WRITE);
		}
	} else {
...
static int exynos_v920_pcie_clock_get(struct pcie_port *pp)
{
	...
	if (exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2) {
		...
		ret_hwacg = exynos_v920_pcie_dbiclk_enable(pci->dev, clks, true);
		if (ret_hwacg)
	...
	int vf_num, u64 base_addr, u64 target_addr, u32 size, u64 vf_bar_size)
{
	...
	if (vf_num <= exynos_pcie->num_vfs) {
		...
		writel(PCIE_ATU_TYPE_MEM, pci->atu_base  + PCIE_ATU_CR1_OUTBOUND0 + offset);

		writel(lower_32_bits(vf_base_addr), pci->atu_base + PCIE_ATU_LOWER_BASE_OUTBOUND0
			+ offset);

		writel(upper_32_bits(vf_base_addr), pci->atu_base + PCIE_ATU_UPPER_BASE_OUTBOUND0
			+ offset);

		writel((vf_base_addr + size - 1), pci->atu_base + PCIE_ATU_LIMIT_OUTBOUND0
			+ offset);
		...
		writel(PCIE_ATU_ENABLE, pci->atu_base  + PCIE_ATU_CR2_OUTBOUND0 + offset);
...
retry:
	...
	if (count >= MAX_TIMEOUT) {
		...
		if (exynos_pcie->linkflushtime > LINK_FLUSH_TIME_NONE) {
			preval = exynos_elbi_read(exynos_pcie, PCIE_GEN_CTRL4);
			val = preval | LINK_FLUSH_TIME_M(
		...
		if (exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2) {
			preval = exynos_elbi_read(exynos_pcie, PCIE_USER_INT_EN);
			val = preval | IRQ_CPL_TIMEOUT_EN;
...
static int exynos_v920_pcie_assert_phy_reset(struct pcie_port *pp)
{
	...
	if (exynos_pcie->phy_ops.phy_config != NULL)
		exynos_pcie->phy_ops.phy_config(pp, &ret, exynos_pcie->fw_update);
	if (!ret)
		return 0;
...
static irqreturn_t exynos_v920_pcie_irq_handler(int irq, void *arg)
{
	...
	if (val & IRQ_LINK_DOWN) {
		...
		exynos_v920_pcie_send_evt(exynos_pcie, STATE_LINK_DOWN_TRY);
				...
				struct platform_device *pdev)
{
	...
	if (exynos_pcie->use_sharability) {
		val = readl(exynos_pcie->sysreg_base + PCIE_SYSREG_SHARABILITY_CTRL +
			PCIE_SYSREG_SHARABLE_RW_OFFSET +
			PCIE_SYSREG_SHARABLE_CH_OFFSET * exynos_pcie->ch_num);
		val |= PCIE_SYSREG_SHARABLE_RW_ENABLE;
		writel(val, exynos_pcie->sysreg_base + PCIE_SYSREG_SHARABILITY_CTRL +
			PCIE_SYSREG_SHARABLE_RW_OFFSET +
			PCIE_SYSREG_SHARABLE_CH_OFFSET * exynos_pcie->ch_num);
							...
							struct pcie_port *pp)
{
	...
	if (of_property_read_u32(np, "link-flush-time",
		...
		exynos_pcie->linkflushtime = LINK_FLUSH_TIME_NONE;
	} else {
...
static int exynos_v920_pcie_probe(struct platform_device *pdev)
{
	...
	dev_info(pci->dev, "%s: Version: %s\n", __func__,
			(exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2) ? "EVT2" : "EVT01");
	...
	if (exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2 &&
		...
		if (!exynos_pcie->pcie_fw_buf) {
			dma_free_coherent(pci->dev, FW_SIZE, exynos_pcie->pcie_fw_buf,
				exynos_pcie->fw_paddr);
			dev_err(pci->dev, "PCIe CH2 DMA alloc fail for FW update.\n");
		} else {
			memcpy(exynos_pcie->pcie_fw_buf, pcie_fw_205_hex, FW_SIZE);
		}
	...
	INIT_LIST_HEAD(&exynos_pcie->ddopslist_head.list);
	...
	if (exynos_pcie->s2mpu_base) {
		if (hyp_call_s2mpu_vgen_v9_protection(
			...
			dev_info(&pdev->dev, "%s Fail s2mpu v9=%08x vgen v9=%08x\n",
				 __func__, exynos_pcie->s2mpu_base, exynos_pcie->vgen_base);
		else
...
static const struct of_device_id exynos_v920_pcie_of_match[] = {
	{ .compatible = "samsung,exynos-v920-evt01-pcie-rc", .data = &exynosautov920_evt01 },
	{ .compatible = "samsung,exynos-v920-evt2-pcie-rc", .data = &exynosautov920_evt2 },
	{},
};
...
void exynos_v920_pcie_poweroff(int ch_num)
{
	...
	if (exynos_pcie->state == STATE_LINK_UP ||
		...
		if (exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2)
			exynos_elbi_write(exynos_pcie, EVT2_PME_ACK_INT_EN, RX_MSG_INT_EN);
		else
			exynos_elbi_write(exynos_pcie, EVT01_PME_ACK_INT_EN, RX_MSG_INT_EN);

		spin_lock_irqsave(&exynos_pcie->conf_lock, flags);
		exynos_v920_pcie_send_pme_turn_off(exynos_pcie);
		exynos_pcie->state = STATE_LINK_DOWN;
		exynos_v920_pcie_send_evt(exynos_pcie, STATE_LINK_DOWN);
		...
		if (lft_sleep > LINK_FLUSH_TIME_NONE) {
			if (lft_sleep <= LINK_FLUSH_TIME_1MS_VALUE)
				lft_sleep = LINK_FLUSH_TIME_1MS_VALUE;

			lft_sleep = LFT_SLEEP_CAL(lft_sleep);
			msleep(lft_sleep);
		}
	...
	u16 evt)
{
	...
	struct exynos_pcie_ext_dd_ops_list *hostddops = NULL;

	list_for_each_entry(hostddops, &exynos_pcie->ddopslist_head.list, list) {
		if (hostddops->chdrv_ops) {
			if (hostddops->chdrv_ops->event_cb) {
				hostddops->chdrv_ops->event_cb(hostddops->pdev, evt);
				dev_info(dev, "%s ch%d vdid(0x%x) vf%d evt(%d)\n",
					__func__, hostddops->ch_num, hostddops->vdid,
					hostddops->vf_num, evt);
			} else {
				dev_err(dev, "%s ch%d vdid(0x%x) vf%d no event_cb\n",
					__func__, hostddops->ch_num, hostddops->vdid,
					hostddops->vf_num);
			}
		} else {
			dev_err(dev, "%s ch%d vdid(0x%x) vf%d no chdrv_ops\n", __func__,
				hostddops->ch_num, hostddops->vdid, hostddops->vf_num);
		}
	}
}
...
static bool exynos_v920_pcie_is_linkup(u8 ch_num)
{
	struct exynos_pcie *exynos_pcie = &g_pcie_v920[ch_num];
...
struct exynos_dd_ops exynos_ddops = {
	.get_dev = exynos_v920_pcie_get_dev,
	.is_linkup = exynos_v920_pcie_is_linkup,
};

int chdrv_dd_register(struct pci_dev *pdev, u8 ch_num, u16 vdid, u16 vf_num,
	struct exynos_dd_ops *host_ops, struct chdrv_rc_dd_ops *host_dd_ops)
{
	struct exynos_pcie *exynos_pcie = &g_pcie_v920[ch_num];
	struct device *dev = exynos_pcie->pci->dev;
	struct exynos_pcie_ext_dd_ops_list *hostddops = NULL;

	hostddops = kzalloc(sizeof(struct exynos_pcie_ext_dd_ops_list),
		GFP_KERNEL);
	...
	hostddops->pdev = pdev;
	hostddops->ch_num = ch_num;
	hostddops->vdid = vdid;
	hostddops->vf_num = vf_num;
	hostddops->chdrv_ops = host_dd_ops;
	host_ops->get_dev = exynos_ddops.get_dev;
	host_ops->is_linkup = exynos_ddops.is_linkup;

	list_add(&hostddops->list, &exynos_pcie->ddopslist_head.list);

	dev_info(dev, "%s added\n", __func__);

	return 0;
}
EXPORT_SYMBOL(chdrv_dd_register);

void chdrv_dd_unregister(u8 ch_num, u16 vdid, u16 vf_num,
	struct chdrv_rc_dd_ops *host_dd_ops)
{
	struct exynos_pcie *exynos_pcie = &g_pcie_v920[ch_num];
	struct device *dev = exynos_pcie->pci->dev;
	struct exynos_pcie_ext_dd_ops_list *hostddops = NULL;
	...
	list_for_each_safe(pos, q, &exynos_pcie->ddopslist_head.list) {
		hostddops = list_entry(pos, struct exynos_pcie_ext_dd_ops_list, list);
		if (hostddops->ch_num == ch_num && hostddops->vdid == vdid
			&& hostddops->vf_num == vf_num) {
			list_del(&hostddops->list);
			kfree(hostddops);
			dev_info(dev, "%s ch%d vdid(0x%x) vf%d host_dd_ops is deleted\n",
				__func__, ch_num, vdid, vf_num);
		}
	}
}
EXPORT_SYMBOL(chdrv_dd_unregister);
...
static int exynos_v920_pcie_resume_noirq(struct device *dev)
{
	...
	if (!exynos_pcie->s2r_if_control) {
		if ((exynos_pcie->ch_num == PCIE_CH2 || exynos_pcie->ch_num == PCIE_CH3) &&
			exynos_pcie->soc_variant == EXYNOSAUTOV920_EVT2 &&
			exynos_pcie->evt2_minor_ver == EVT20) {
			...
			for (i = 0; i < PCIE_RC_RETRY; i++) {
				...
				if (exynos_pcie->phy_ops.phy_config != NULL &&
					exynos_pcie->fw_update && exynos_pcie->ch_num == PCIE_CH2)
					exynos_pcie->phy_ops.phy_config(pp, &ret,
						exynos_pcie->fw_update);

				if (!ret)
					dev_err(dev, "FW update Fail [%d/10]\n", i);
			...
			if (exynos_pcie->phy_fw_ver == FW_VER_205) {
				...
				dev_err(dev, "PCIe PHY is not configured. FW ver:%x\n",
					exynos_pcie->phy_fw_ver);
			}
		} else {
			/* For 4L phy */
