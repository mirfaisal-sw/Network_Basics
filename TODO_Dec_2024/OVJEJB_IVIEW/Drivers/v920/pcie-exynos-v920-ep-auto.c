...
void exynos_v920_pcie_ep_phyinit(struct exynos_ep_pcie *pcie_ep)
{
	...
	if (pcie_ep->soc_variant == EXYNOSAUTOV920_EVT2) {
		...
		pcie_ep->phy_ops.phy_config = exynos_v920_evt2_pcie_ep_phy_config;
		...
		pcie_ep->phy_ops.phy_all_pwrdn_clear = exynos_v920_pcie_ep_phy_all_pwrdn_clear;
	...
	u8 vfunc_no, int index, int type, u64 cpu_addr, u64 pci_addr, u64 size)
{
	...
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_LOWER_BASE,
				 lower_32_bits(cpu_addr));
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_UPPER_BASE,
				 upper_32_bits(cpu_addr));
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_LOWER_LIMIT,
				 lower_32_bits(limit_addr));
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_UPPER_LIMIT,
				 upper_32_bits(limit_addr));
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_LOWER_TARGET,
				 lower_32_bits(pci_addr));
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_UPPER_TARGET,
	...
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_REGION_CTRL1,
		val);
	exynos_v920_pcie_ep_writel_ob_unroll(pci, index, PCIE_ATU_UNR_REGION_CTRL2,
				 PCIE_ATU_ENABLE);

	if (vfunc_no == 0)
		exynos_v920_pcie_ep_writel_ob_unroll(pci, index, 0x1C, 0x0);
	else if (vfunc_no > 0)
		exynos_v920_pcie_ep_writel_ob_unroll(pci, index, 0x1C, BIT(31) |
			(vfunc_no-1));
		     ...
		     phys_addr_t phys_addr, u64 pci_addr, size_t size)
{
	...
	mutex_lock(&epc->lock);
	ret = exynos_v920_pcie_ep_map_addr(ep, func_no, vfunc_no, phys_addr, pci_addr,
				 size);
...
static int exynos_v920_pcie_ep_setup_interrupt(struct exynos_ep_pcie *pcie_ep)
{
	...
	writel(PCIE_EP_IRQMASK, pcie_ep->elbi_base + offset);

	writel(PCIE_EP_IRQMASK, pcie_ep->elbi_base + PCIE_RX_MSG_INT_EN);

	dev_info(NULL, "[%s]: Interrupt Setup Done\n", __func__);
	return 0;
}
							...
							int bar_no, int pf_no, int vf_no)
{
	...
	struct pci_epf_bar *epf_bar = &pcie_ep->bar[bar_no].epf_bar;
	int ret = 0;

	ret = pci_epc_set_bar(epc, 0, 0, epf_bar);
	dev_info(dev, "%s BAR%d PA:0x%llx, VA:0x%p, SIZE:0x%lx FLAGS:0x%x ret:%d\n",
...
static void exynos_v920_pcie_ep_bar_setting(struct exynos_ep_pcie *pcie_ep)
{
	...
	if (pcie_ep->ext_node.chdrv_ops != NULL) {
		if (pcie_ep->ext_node.chdrv_ops->set_bar_mem != NULL) {
			for (i = 0 ; i < MAX_VF_NUM + 1 ; i++) {
				if (pcie_ep->ext_node.vf_num[i] == true) {
					ret = pcie_ep->ext_node.chdrv_ops->set_bar_mem(
						pcie_ep->ch_num, pcie_ep->ext_node.vdid, i, dev,
						pcie_ep->use_sysmmu);
	...
	for (i = BAR_0 ; i <= BAR_5 ; i++) {
		...
		reg_sriov =
			PCIE_SHADOW_REG_POS + PCIE_SRIOV_CAP_POS + PCI_SRIOV_BAR + (i*4);
		...
		case BAR_TYPE_DOORBELL:
			...
			case CH0_4L:
				epf_bar->phys_addr = PCIE_0_DOORBELL_BASE_ADDRESS;
				break;
			case CH1_4L:
				epf_bar->phys_addr = PCIE_1_DOORBELL_BASE_ADDRESS;
				break;
			case CH2_2L:
				epf_bar->phys_addr = PCIE_2_DOORBELL_BASE_ADDRESS;
				break;
			case CH3_2L:
				epf_bar->phys_addr = PCIE_3_DOORBELL_BASE_ADDRESS;
				break;
			default:
			...
			if (pcie_ep->use_sysmmu && (pcie_ep->bar[i].is_mapped == false)) {
				dev_info(dev, "%s: sysmmu map BAR CH%d DB sz:0x%lx\n",
					__func__, pcie_ep->ch_num, epf_bar->size);
				domain = iommu_get_domain_for_dev(dev);
				iommu_map(domain, epf_bar->phys_addr, epf_bar->phys_addr,
					epf_bar->size, IOMMU_READ | IOMMU_WRITE);
				pcie_ep->bar[i].is_mapped = true;
			} else {
				dev_info(dev, "%s: DB use_sysmmu(%d) is_mapped(%d)\n",
					__func__, pcie_ep->use_sysmmu, pcie_ep->bar[i].is_mapped);
			}
			break;
		case BAR_TYPE_MEMORY_ALLOC:
		case BAR_TYPE_MEMORY_RESERVED:
		case BAR_TYPE_MEMORY_SFR:
			/* Need to add ops to get memory address data from SCD_EP driver	*/
			if (pcie_ep->ext_node.chdrv_ops != NULL) {
				if (pcie_ep->ext_node.chdrv_ops->get_bar_mem != NULL) {
					pcie_ep->ext_node.chdrv_ops->get_bar_mem(pcie_ep->ch_num,
						pcie_ep->ext_node.vdid, 0, epf_bar->barno,
						&epf_bar->phys_addr, epf_bar->addr,
						pcie_ep->bar[i].type, pcie_ep->bar[i].size);
				} else {
					epf_bar->size = 0;
					size = 0;
				}
		...
		dev_info(dev, "%s BAR%d VA:0x%p PA:0x%llx\n",
			__func__, epf_bar->barno, epf_bar->addr, epf_bar->phys_addr);
		...
		if (is_setup) {
			if (pcie_ep->bar[i].mem_type == BAR_ADDRESS_64) {
				...
				dw_pcie_writel_dbi(pci, reg + 4, upper_32_bits(size));
				dw_pcie_writel_dbi(pci, reg1 + 4, 0);
				dw_pcie_writel_dbi(pci, reg_sriov + 4, upper_32_bits(size));
			...
			if (epf_bar->size > 0)
				exynos_v920_pcie_ep_set_bar_inbound(pcie_ep, i, 0, 0);
		}
	}
...
start_linkup:
	if (pcie_ep->linkup_ready == 1) {
		...
		if (pcie_ep->soc_variant == EXYNOSAUTOV920_EVT01) {
			if ((readl(pcie_ep->sysreg_base + pcie_ep->sysreg_offset)
						& 0xf0) >> 4 == 0x1) {
			...
			if (pcie_ep->use_sysmmu && pcie_ep->bar[i].is_mapped) {
				if (pcie_ep->bar[i].type == BAR_TYPE_DOORBELL) {
					dev_info(dev, "%s SYSMMU unmap BAR CH%d DB sz:0x%lx\n",
						__func__, pcie_ep->ch_num, epf_bar->size);
					domain = iommu_get_domain_for_dev(dev);
					iommu_unmap(domain, epf_bar->phys_addr, epf_bar->size);
						...
						u32 value, u32 mask)
{
	...
	if (mask == perst_n_0)
		...
		if (pcie_ep->state == STATE_LINK_UP ||
			...
			exynos_v920_pcie_ep_send_evt(pcie_ep, STATE_LINK_DOWN_ABNORMAL);
			dev_info(&pdev->dev,
...
static irqreturn_t exynos_v920_pcie_ep_irq_handler(int irq, void *arg)
{
	...
	if ((pcie_ep->state == STATE_LINK_DOWN) && (pcie_ep->phy_ops.phy_all_pwrdn_clear != NULL))
		pcie_ep->phy_ops.phy_all_pwrdn_clear(pcie_ep->ep_phyinfo);
...
static irqreturn_t exynos_v920_pcie_ep_doorbell_irq_handler(int irq, void *arg)
{
	...
	struct platform_device *pdev = pcie_ep->pdev;
	struct device *dev = &pdev->dev;
	struct chdrv_ep_dd_ops *chdrv_ops = NULL;
	...
	if (db_data & PCIE_EP_IRQMASK) {
		chdrv_ops = pcie_ep->ext_node.chdrv_ops;

		if (chdrv_ops != NULL) {
			if (chdrv_ops->rvc_doorbell != NULL) {
				ret = chdrv_ops->rvc_doorbell(pcie_ep->ch_num,
					pcie_ep->ext_node.vdid, db_data);
				if (ret)
					dev_err(dev,
					    ...
					    int dma_dir, int result)
{
	...
	chdrv_ops = pcie_ep->ext_node.chdrv_ops;
	if (chdrv_ops && chdrv_ops->dma_resolve)
		chdrv_ops->dma_resolve(pcie_ep->ch_num, pcie_ep->ext_node.vdid,
				       0, dma_dir, ll_start, ll_cnt, result);
	else
		dev_err(dev, "%s: no ops of ext node\n", __func__);
}
...
static int exynos_v920_pcie_ep_msi_alloc(struct exynos_ep_pcie *pcie_ep)
{
	...
	if (pcie_ep->msi_region.msi_va) {
		...
		dev_info(dev, "%s already set. PA:0x%llx VA:0x%p PCI_ADDR:0x%llx\n",
			__func__, pcie_ep->msi_region.msi_pa, pcie_ep->msi_region.msi_va,
	...
	pcie_ep->msi_region.msi_pciaddr = (pcie_ep->msi_region.umsi_pci_addr << 32)
	...
	ret = exynos_v920_pcie_ep_epc_map_addr(epc, 0, 0, pcie_ep->msi_region.msi_pa,
		pcie_ep->msi_region.msi_pciaddr,
	...
	size_t size, int dma_num)
{
	...
	writel(0x1, pcie_ep->dma_base + PCIE_DMA_WR_EN);
	/* enable DMA power */
	writel(0x1, pcie_ep->dma_base + PCIE_DMA_WR_PWR_EN + PWR_EN_OFFSET(dma_num));
	writel(0x0, pcie_ep->dma_base + PCIE_DMA_WR_INT_MASK);
	writel(0x0, pcie_ep->dma_base + PCIE_DMA_WR_INT_CLEAR);
	writel(0x0 | CH_CTRL1_LLE | CH_CTRL1_LIE | CH_CTRL1_DMA_RESERVED5, pcie_ep->dma_base +
		PCIE_DMA_CH_CTRL1 + dma_num * PCIE_DMA_CH_OFFSET);
	...
	size_t size, int dma_num)
{
	...
	writel(0x1, pcie_ep->dma_base + PCIE_DMA_RD_EN);
	/* enable DMA power */
	writel(0x1, pcie_ep->dma_base + PCIE_DMA_RD_PWR_EN + PWR_EN_OFFSET(dma_num));
	writel(0x0, pcie_ep->dma_base + PCIE_DMA_RD_INT_MASK);
	writel(0x0, pcie_ep->dma_base + PCIE_DMA_RD_INT_CLEAR);

	writel(0x0 | CH_CTRL1_LLE | CH_CTRL2_LIE | CH_CTRL2_DMA_RESERVED5, pcie_ep->dma_base +
		PCIE_DMA_CH_CTRL2 + dma_num * PCIE_DMA_CH_OFFSET);
 ...
 * @retval{ret, int, 0, -ERANGE~0, <0}
 */
int exynos_v920_pcie_ep_write_test(struct exynos_ep_pcie *pcie_ep, u64 dst_pci_addr,
	int dma_num)
{
	struct platform_device *pdev = pcie_ep->pdev;
	struct device *dev = &pdev->dev;
	...
	ret = exynos_v920_pcie_ep_epc_map_addr(epc, 0, 0, phys_addr, dst_pci_addr, size);
	if (ret) {
		dev_err(dev, "failed to map address\n");
		ret = -ENOMEM;
		goto err_addr;
 ...
 * @retval{ret, int, 0, -ERANGE~0, <0}
 */
int exynos_v920_pcie_ep_read_test(struct exynos_ep_pcie *pcie_ep, u64 src_pci_addr,
	u32 size, int dma_num)
{
	...
	ret = exynos_v920_pcie_ep_epc_map_addr(epc, 0, 0, phys_addr, src_pci_addr, size);
	if (ret) {
		dev_err(dev, "failed to map address\n");
		ret = -ENOMEM;
		goto err_addr;
	...
	case 2:
		if (sscanf(buf, "%d %d", &op_num, &dma_num) == 2) {
			if (pcie_ep->state != STATE_LINK_UP) {
				...
				dev_err(dev, "Sysmmu is enabled, use SCD TEST for DMA Test.\n");
				return -1;
			}
...
static int exynos_v920_pcie_ep_probe(struct platform_device *pdev)
{
	...
	if (ch_num >= MAX_EP_NUM) {
		dev_err(dev, "ch_num value leadsexceeds the boundary value of g_pcie_ep_v920\n");
		return -EINVAL;
	}
			...
			int type, u16 vf_num, u64 cpu_addr, u64 pci_addr, u32 size)
{
	...
	if (vf_num > 0)
		dev_info(pci->dev, "[%s] VF num : %d (TBD)\n", __func__, vf_num);
...
static void exynos_v920_pcie_ep_send_evt(struct exynos_ep_pcie *pcie_ep,
	u16 evt)
{
	struct exynos_ep_ext_node *extnode;
	struct device *dev;
	int i;

	extnode = &pcie_ep->ext_node;
	dev = pcie_ep->pci->dev;

	if (extnode->chdrv_ops != NULL) {
		if (extnode->chdrv_ops->event_cb != NULL) {
			for (i = 0 ; i < MAX_VF_NUM + 1 ; i++) {
				if (extnode->vf_num[i] == true) {
					extnode->chdrv_ops->event_cb(
						pcie_ep->ch_num,
						extnode->vdid, i, evt);
					dev_info(dev,
						 "%s vdid(0x%x) vf%d evt(%d)\n",
						 __func__, extnode->vdid, i,
						 evt);
				}
			}
		}
...
static int exynos_v920_pcie_ep_send_msi(u8 ch_num, u16 vdid, u16 vf_num,
	u8 bar_num, u32 data)
{
	int ret = 0;
	struct exynos_ep_pcie *pcie_ep = &g_pcie_ep_v920[ch_num];
...
static int exynos_v920_pcie_ep_set_obatu(u8 ch_num, u16 vf_num, u16 vdid,
	u8 bar_num, dma_addr_t remote_dma_addr, size_t size, phys_addr_t *ob_phy_addr,
	void __iomem *ob_dest_vaddr)
{
	...
	if (vf_num == 0) {
		...
		exynos_pcie_ep_atu_set(ep, ATU_OB, PCIE_ATU_TYPE_MEM, vf_num, *ob_phy_addr,
			remote_dma_addr, size);
	}
...
static void exynos_v920_pcie_ep_reset_obatu(u8 ch_num, u16 vdid, u16 vf_num,
	u8 bar_num, size_t size, phys_addr_t ob_phy_addr, void __iomem *ob_dest_vaddr)
{
	struct exynos_ep_pcie *pcie_ep = &g_pcie_ep_v920[ch_num];
	struct dw_pcie *pci = pcie_ep->pci;
	struct dw_pcie_ep *ep = &pci->ep;
...
static int exynos_v920_pcie_ep_edma_read_once(u8 ch_num, u16 vdid, u16 vf_num,
					      u8 dma_num,
...
#if	DMA_LOG_PRINT_EP
	dev_info(&pdev->dev, "%s ch%d dma read once on [%llx -> %llx] size:%lx\n",
...
#if DMA_CONT_MODE_EP
	ret = exynos_v920_pcie_ep_read(pcie_ep, remote_src_addr, local_dest_addr,
		size, dma_num);
#else
	ret = exynos_v920_pcie_ep_read_ll(pcie_ep, remote_src_addr, local_dest_addr,
		size, dma_num);
#endif
...
static int exynos_v920_pcie_ep_edma_write_once(u8 ch_num, u16 vdid, u16 vf_num,
					       u8 dma_num,
...
#if	DMA_LOG_PRINT_EP
	dev_info(&pdev->dev, "%s ch%d dma write once on [%llx -> %llx] size:%lx\n",
...
struct exynos_ep_dd_ops exynos_ep_ddops = {
	.get_dev = exynos_v920_pcie_ep_get_dev,
	.send_msi = exynos_v920_pcie_ep_send_msi,
	.set_obatu = exynos_v920_pcie_ep_set_obatu,
	.reset_obatu = exynos_v920_pcie_ep_reset_obatu,
	.edma_read_once = exynos_v920_pcie_ep_edma_read_once,
	.edma_write_once = exynos_v920_pcie_ep_edma_write_once,
	.msi_alloc = exynos_v920_pcie_ep_msi_alloc_map,
};

int chdrv_dd_ep_register(u8 ch_num, u16 vdid, u16 vf_num,
	struct exynos_ep_dd_ops *dd_ops, struct chdrv_ep_dd_ops *ep_dd_ops)
{
	struct exynos_ep_pcie *pcie_ep = &g_pcie_ep_v920[ch_num];
	struct exynos_ep_ext_node *epextnode = NULL;

	epextnode = &pcie_ep->ext_node;

	if (ep_dd_ops == NULL) {
		dev_err(NULL, "%s ERROR ep_dd_ops is NULL\n", __func__);
		return -EINVAL;
	}

	if (epextnode->is_activated == false) {
		epextnode->ch_num = ch_num;
		epextnode->vdid = vdid;
		epextnode->chdrv_ops = ep_dd_ops;
		epextnode->is_activated = true;
		dev_info(NULL, "%s ch%d vdid(0x%x) 1st registration\n", __func__,
			ch_num, vdid);
	}

	epextnode->vf_num[vf_num] = true;
	dd_ops->get_dev = exynos_ep_ddops.get_dev;
	dd_ops->edma_read_once = exynos_ep_ddops.edma_read_once;
	dd_ops->edma_write_once = exynos_ep_ddops.edma_write_once;
	dd_ops->reset_obatu = exynos_ep_ddops.reset_obatu;
	dd_ops->send_msi = exynos_ep_ddops.send_msi;
	dd_ops->set_obatu = exynos_ep_ddops.set_obatu;
	dd_ops->msi_alloc = exynos_ep_ddops.msi_alloc;

	dev_info(NULL, "%s ch%d vdid(0x%x) vf:%d\n", __func__,
		ch_num, vdid, vf_num);

	return 0;
}
EXPORT_SYMBOL(chdrv_dd_ep_register);

void chdrv_dd_ep_unregister(u8 ch_num, u16 vdid, u16 vf_num,
	struct chdrv_ep_dd_ops *ep_dd_ops)
{
	struct exynos_ep_pcie *pcie_ep = &g_pcie_ep_v920[ch_num];
	struct exynos_ep_ext_node *epextnode = NULL;
	...
	if (epextnode->is_activated == false) {
		dev = pcie_ep->pci->dev;
		dev_info(dev, "%s no ext_node\n", __func__);
		return;
	}

	if (ch_num == epextnode->ch_num && vdid == epextnode->vdid)
		epextnode->vf_num[vf_num] = false;

	for (i = 0 ; i < MAX_VF_NUM + 1 ; i++) {
		if (epextnode->vf_num[i] == true) {
			dev = pcie_ep->pci->dev;
			dev_info(dev, "%s still alive\n", __func__);
			return;
		}
	}

	epextnode->chdrv_ops = NULL;
	epextnode->is_activated = false;
}
EXPORT_SYMBOL(chdrv_dd_ep_unregister);
...
static const struct of_device_id exynos_pcie_ep_of_match[] = {
	{ .compatible = "samsung,exynos-v920-evt01-pcie-ep", .data = &exynosautov920_evt01 },
	{ .compatible = "samsung,exynos-v920-evt2-pcie-ep", .data = &exynosautov920_evt2 },
	{},
};
