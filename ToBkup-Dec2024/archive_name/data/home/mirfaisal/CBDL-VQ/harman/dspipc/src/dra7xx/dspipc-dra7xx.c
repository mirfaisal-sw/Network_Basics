...
*****************************************************************************/
/**
 * @file dspipc-dra7xx.c
 *
 * This is the implementation for the dra7xx dspipc chipset layer.
 * for Jacinto 5
 *
 * @author Andreas Abel
 * @date   25.10.2010
...
#include "coff.h"
#include "dra7xx_defs.h"
#include "dsp/DSPIPCTypes.h"
#include "dsp/dra7xxcommstructs.h"  /* description of the communication to dra7xx */
#include "dsp/dra7xxemifipcdebug.h" /* filename array for assert messages */
 ...
 */
#define QNX_INFO_SYM _QNX_info_dspipc_dra7xx
#endif
 ...
 * is used a
 */
#define DRA7XX_CONV_DSP2HOST(x, y) ((uint32_t)(y) - (uint32_t)NULL) /* DSP MemAdr is now used for internal mepped Memory !!! */
 ...
 * an assert was taken
 */
static DSPIPCDra7xxFileNames DSPIPCdra7xxFileNameArray[] = {
        { FNUM_DRA7XXEMIFIPC_C, "DRA7XXEMIFIPC.c" },
        { 0, NULL }
};
 ...
 * @enum DRA7XX_SUBOPTIONS_E
 */
enum DRA7XX_SUBOPTIONS_E {
        DRA7XX_SUB_OPT_FILENAME = 0,           /* Index of the 'filename' token inside the suboption string array */
        DRA7XX_SUB_OPT_VERBOSE = 1,            /* Index of the 'verbose' token inside the suboption string array */
        DRA7XX_SUB_OPT_INTPRIO = 2,            /* Index of the 'intprio' token inside the suboption string array */
        DRA7XX_SUB_OPT_SUPPRESS = 3,           /* Index of the 'suppress' token inside the suboption string array */
        DRA7XX_SUB_OPT_SIMULATOR = 4,          /* Index of the 'simulator' token inside the suboption string array */
        DRA7XX_SUB_OPT_LOADER = 5,             /* Index of the 'loader' token inside the suboption string array */
        DRA7XX_SUB_OPT_COPYMODE = 6,           /* Index of the 'copymode' token inside the suboption string array */
        DRA7XX_SUB_OPT_DMACHAN = 7,            /* Index of the 'dmachan' token inside the suboption string array */
        DRA7XX_SUB_OPT_INT = 8,                /* Index of the 'int' token inside the suboption string array */
        DRA7XX_SUB_OPT_BASE = 9,               /* Index of the 'base' token inside the suboption string array */
        DRA7XX_SUB_OPT_DUMMYBL = 10,           /* Index of the 'dummybl' token inside the suboption string array */
        DRA7XX_SUB_OPT_MEMSIZE = 11,           /* Index of the 'memsize' token inside the suboption string array */
        DRA7XX_SUB_OPT_DSP_STARTUP_PARAM = 12, /* Index of the 'startupParam' token inside the suboption string array */
        DRA7XX_SUB_OPT_CLOCK_ENABLE = 13,      /* Index of the 'clockenable' token inside the suboption string array */
...
#ifdef VERBOSE_ENABLE
        DRA7XX_SUB_OPT_NOCOMM = 16 /* Index of the 'nocomm' token inside the suboption string array */
#endif
...
int dspipc_dra7xx_loadFirmware(const deviceTable_t* pDevice);
int dspipc_dra7xx_startFirmware(const deviceTable_t* pDevice);
int dspipc_dra7xx_startApplication(const deviceTable_t* pDevice);
int dspipc_dra7xx_startCommunication(const deviceTable_t* pDevice);
int dspipc_dra7xx_registerShmObj(const deviceTable_t* pDevice, const char* pName,
    uint32_t physAddr, uint32_t size);
int dspipc_dra7xx_deregisterShmObj(const deviceTable_t* pDevice, const char* pName);
int dspipc_dra7xx_send(subDeviceTable_t* pSubDevice);
int dspipc_dra7xx_send_drain(subDeviceTable_t* pSubDevice, drainSendMode_t mode);
int dspipc_dra7xx_intrThread(deviceTable_t* pDevice);
int dspipc_dra7xx_close(deviceTable_t* pDevice);
int dspipc_dra7xx_getDspSwVers(const deviceTable_t* pDevice, dspipc_dspSwVers_t* pVersion);
int dspipc_dra7xx_resetState(const deviceTable_t* pDevice, resetType_t type);

/*==== internal helper functions == */
static void dra7xxRegWrite32(const dspIpcChipset_t* pChipset, uint32_t RegModSel, uint32_t addr, volatile uint32_t val);
static void dra7xxRegRead32(const dspIpcChipset_t* pChipset, uint32_t RegModSel, uint32_t addr, volatile uint32_t* pVal);
static int dra7xxAttach(dspIpcChipset_t* pChipset);
static int dra7xxSendBootTable(dspIpcChipset_t* pChipset, int bootFd);
static int dra7xxSetConfigState(dspIpcChipset_t* pChipset, dra7xxConfigState_t newState);
#ifdef SIMULATOR
static int dra7xxPrepareInterrupt(const deviceTable_t* pDevice);
#endif
static int dra7xxInterruptUnmask(const deviceTable_t* pDevice);
static void dra7xxSetInterrupt(dspIpcChipset_t* pChipset);
static int dra7xxMemWrite(const dspIpcChipset_t* pChipset, const uint32_t addr, const uint32_t value);
static int dra7xxMemRead(const dspIpcChipset_t* pChipset, const uint32_t addr, volatile uint32_t* dest);
int dra7xxAccessMem32(dspIpcChipset_t* pChipset,
    uint32_t addr,
    uint32_t numLongWords,
    memAccessDir_t dir,
    uint32_t* pVal);
int dra7xxAccessMem8(dspIpcChipset_t* pChipset,
    uint32_t addr,
    uint32_t numBytes,
    memAccessDir_t dir,
    uint8_t* pVal);
static void dra7xxDumpMessage(const char* pString, DSPIPCMessageQueueEntry* pMsg);
static const char* CmdToString(uint32_t cmd);
#ifdef DUMP_DATA
static void dra7xxDumpData(const char* pString, void* pMsg, unsigned int size);
#endif
static uint32_t dra7xx_get_queue_index(dspIpcChipset_t* pChipset, uint32_t queueOff,
    int32_t* pRead, int32_t* pWrite);
static uint32_t dra7xx_set_queue_index(dspIpcChipset_t* pChipset, uint32_t queueOff,
    const int32_t* pRead, const int32_t* pWrite);
static int dra7xxSendMsg(dspIpcChipset_t* pChipset, DSPIPCMessageQueueEntry* pMsg, uint32_t pPhysMsg,
    const dra7xxSendIndicator_t setInterrupt);
...
static int dra7xxDrain(subDeviceTable_t* pSubDevice);
...
static void dra7xxMemWrite_oc(const dspIpcChipset_t* pChipset, uint32_t addr, const uint32_t* value, uint32_t numLongWords);
static void dra7xxMemRead_oc(const dspIpcChipset_t* pChipset, uint32_t addr, uint32_t* dest, uint32_t numLongWords);
static int dra7xxAccessMem32_DMA(dspIpcChipset_t* pChipset, uint32_t dps_addr, uint32_t numLongWords, memAccessDir_t dir, uint32_t* pVal, uint32_t pVal_phys);

static int setMemSelAdr(dspIpcChipset_t* pChipset, uint32_t addr);
static int dra7xx_mailboxInitDrv(void);
static int dra7xx_mailboxExitDrv(void);
static int dra7xx_mailbox_queue_status(dspIpcChipset_t* pChipset);
...
int dspipc_dra7xx_init(deviceTable_t* pDevice, char* pOptions)
{
        ...
        if (pDevice != NULL) {
                ...
                pDevice->loadFWFunc = dspipc_dra7xx_loadFirmware;
                pDevice->startFWFunc = dspipc_dra7xx_startFirmware;
                pDevice->startAppFunc = dspipc_dra7xx_startApplication;
                pDevice->startComFunc = dspipc_dra7xx_startCommunication;
                pDevice->registerShmObjFunc = NULL;
                pDevice->deregisterShmObjFunc = NULL;
                pDevice->sendFunc = dspipc_dra7xx_send;
                pDevice->sendStreamDataFunc = NULL;
                pDevice->sendDrainFunc = dspipc_dra7xx_send_drain;
                pDevice->intThread = dspipc_dra7xx_intrThread;
                pDevice->closeFunc = dspipc_dra7xx_close;
                pDevice->dumpStateFunc = NULL;
                pDevice->getDspSwVers = dspipc_dra7xx_getDspSwVers;
                pDevice->resetState = dspipc_dra7xx_resetState;
                pDevice->intNumber = -1;
                ...
                if (pChipset != NULL) {
                        ...
                        pChipset->dspRootAddr = DRA7XX_ROOTPOINTER_ADDR; /* DRA7XX_ROOTPOINTER_ADDR   ROOTPOINTER_ADDR */
                        pChipset->regModCtrl = NULL;
                        pChipset->regModConf = NULL;
                        pChipset->verbose = 0;
                        pChipset->suppress = 0;
                        pChipset->heartBeatPeriodeMs = 0;
                        pChipset->configState = DRA7XX_NOT_INITIALIZED;
                        ...
                        if (pOptions != NULL) {
                                while (*pOptions != '\0') {
                                        if ((opt = getsubopt(&pOptions, (const char* const*)subOpts, &pValue)) != -1) {
...
#ifdef VERBOSE_ENABLE
                                                case DRA7XX_SUB_OPT_NOCOMM:
                                                        hb_slogf_error("%s: DO NOT start Communication !!!!!!", __FUNCTION__);
                                                        opt_nocomm++;
                                                        break;
#endif

                                                case DRA7XX_SUB_OPT_DSP_STARTUP_PARAM:
                                                        value = strtoul(pValue, NULL, 0);
#ifdef VERBOSE_ENABLE
                                                        if (pChipset->verbose > 1) {
                                                ...
                                                case DRA7XX_SUB_OPT_CLOCK_ENABLE:
                                                        value = strtoul(pValue, NULL, 0);
#ifdef VERBOSE_ENABLE
                                                        if (pChipset->verbose > 1) {
                        ...
                        if (irqMailboxNum == 0) {
                                ret = dra7xx_mailboxInitDrv();
                                hb_slogf_error("%s: dra7xx_mailboxInitDrv return code is :%d", __FUNCTION__, ret);
                        }

                        if (irqMailboxNum < 0) {
                                irqMailboxNum = pChipset->pDevice->intNumber;
                        }
                        pChipset->pDevice->intNumber = irqMailboxNum;

                        if (retValue == 0) {
                                if (dra7xxAttach(pChipset) == 0) {
                                        if (pChipset->suppress == 0) /* Download of DSP-Application should be done */
                                        {

                                                mem_val = 0x00000003;
                                                dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, RM_DSP_RSTCTRL, mem_val); /* Read RM_DSP_RSTST Register */

                                                /* hard reset DSP */
                                                hb_slogf_error("%s: step 1) Reset", __FUNCTION__);

                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKSTCTRL, &mem_val); /* Read CM_DSP_CLKSTCTRL Register                   */
                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKCTRL, &mem_val);   /* Read CM_DSP_CLKCTRL Register */

                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, PM_DSP_PWRSTCTRL, &mem_val); /* Read PM_DSP_PWRSTCTRL Register */
                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, PM_DSP_PWRSTST, &mem_val);   /* Read PM_DSP_PWRSTST Register */
                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, RM_DSP_RSTCTRL, &mem_val);   /* Read RM_DSP_RSTCTRL Register                   */
                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, RM_DSP_RSTST, &mem_val);     /* Read RM_DSP_RSTST Register */

                                                /* Power up DSP */
                                                dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKSTCTRL, 0x2);
                                                dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKCTRL, 0x1);
                                                /* Check whether GFCLK is gated or not */
                                                do {
                                                        mdelay(1);
                                                        count++;
                                                        dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKSTCTRL, &mem_val); /* Read RM_DSP_RSTST Register */
                                                } while (((mem_val & 0x100) != 0x100) && (count < 100));

                                                if ((mem_val & 0x100) != 0x100) {
                                                        hb_slogf_error("%s: DSP clock is not actived !!!", __FUNCTION__);
                                                } else {
                                                        hb_slogf_error("%s: DSP clock is actived !!!", __FUNCTION__);
                                                }

                                                count = 0;
                                                /* Set Local Reset on DSP */
                                                mem_val = 0x00000001; /* (mem_val) | MDCTL_LRST; */
                                                dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, RM_DSP_RSTCTRL, (mem_val));
...
static int dra7xx_mailbox2_probe(struct platform_device* pdev)
{
        hb_slogf_error("%s: begin to get irq", __FUNCTION__);
        irqMailboxNum = platform_get_irq(pdev, 0);
        if (irqMailboxNum < 0) {
                dev_err(&pdev->dev, "no irq resource for mailbox2.\n");
                return irqMailboxNum;
        }
        hb_slogf_error("%s: get irq:%d", __FUNCTION__, irqMailboxNum);

        return 0;
}
static int dra7xx_mailbox2_remove(struct platform_device* pdev)
{
        hb_slogf_error("%s: called", __FUNCTION__);
        return 0;
}

static const struct of_device_id dra7xx_mailbox2_of_match[] = {
        {
            .compatible = "ti,omap-dspipc-mailbox",
        },
};

static struct platform_driver dra7xx_mailbox2_driver = {
        .probe = dra7xx_mailbox2_probe,
        .remove = dra7xx_mailbox2_remove,
        .driver = {
            .name = "dspipc-mailbox",
            .owner = THIS_MODULE,
            .of_match_table = of_match_ptr(dra7xx_mailbox2_of_match),
        },
};
...
{
        hb_slogf_error("%s: called", __FUNCTION__);
        return platform_driver_register(&dra7xx_mailbox2_driver);
}

static int dra7xx_mailboxExitDrv(void)
{
        hb_slogf_error("%s: called", __FUNCTION__);
        platform_driver_unregister(&dra7xx_mailbox2_driver);
        return 0;
}
...
#endif
/**
 *  This function is attaching the chipset to the dra7xx dsp
 *

 *  @param pChipset Pointer to the dra7xx chipset layer administration structure
 *  @return int 0 if everything was ok, -1 if an error has occurred
 */
static int dra7xxAttach(dspIpcChipset_t* pChipset)
{
...
#define DO_INIT_PRINTOUTS
                                ...
                                if ((pChipset->memAddr != NULL) && (pChipset->regModConf != NULL)) {
                                        ...
                                        if (strcmp(pDevice->pDeviceName, "dsp") == 0) {
                                                hb_slogf_error("%s: register maibox-2 messageQueue0 for DSP=>A8", __FUNCTION__);
                                                /* register maibox-0 for DSP=>A8 */
                                                dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(0), MAILBOX_IRQENABLE_NEWMSG_SET(0));
                                        } else if (strcmp(pDevice->pDeviceName, "m3") == 0) {
                                                hb_slogf_error("%s: register maibox-2 messageQueue4 for M3=>A8", __FUNCTION__);
                                                /* register maibox-4 for M3=>A8 */
                                                dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(0), MAILBOX_IRQENABLE_NEWMSG_SET(8));
                                        }
...
/**
 *  This function downloads the firmware to the dra7xx dsp
 *
 *  @param pDevice Pointer to the device table structure
 *  @return int 0 if everything was ok, -1 if an error has occurred
 */
        ...
        if ((pChipset->regModCtrl != NULL) || (pChipset->simuFd != -1)) {
                if (pChipset->suppress == 0) /* dev-dspipc is doing the download */
                {
                        ...
                        if (pChipset->dummyBL != 0) {
                                ...
                                dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, DCM_DSPBOOTADDR, pChipset->memAddr_phys_Const);
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, DCM_DSPBOOTADDR, &mem_val);
                                ...
                                for (i = 0; i < 32; i++) {
                                        (void)dra7xxMemWrite(pChipset, (pChipset->memShiftDummyBL - (4 * (i + 1))), 0x00000000);
                                }
                                ...
                                for (i = 0; i < 32; i++) {
                                        (void)dra7xxMemWrite(pChipset, (i * 4), 0x00000000);
                                }

                                (void)dra7xxMemWrite(pChipset, 0x00000000, 0xA120);

                                for (i = 0; i < 32; i++) {
                                        (void)dra7xxMemRead(pChipset, (i * 4), &mem_val);
                                }
...
int dspipc_dra7xx_startApplication(const deviceTable_t* pDevice)
{
        ...
        if (pChipset->suppress == 0) /* suppress=0 make Download */
        {
                /*Reset the variable */
                value = 0;
                if (dra7xxAccessMem32(pChipset,
                        DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspRootAddr) + offsetof(struct _DSPIPCRootPointer, magic),
                        1, MEM_WRITE, &value)
                    != 0) {
                ...
                if ((hFd != -1)) {
                        sendResult = dra7xxSendBootTable(pChipset, hFd);
                        if (sendResult != 0) {
        ...
        if (dra7xxAccessMem32(pChipset,
                DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspRootAddr) + offsetof(struct _DSPIPCRootPointer, devDspIpcCmdLineParam),
                1, MEM_WRITE, &(pChipset->DspStartupParam))
            != 0) {
        ...
        if (sendResult == 0) /* If download was successful */
        {
                ...
                for (i = 0; i < 3000; i++) {
                        ...
                        if(value != 0xBEEFDEAD) {
                                if (dra7xxAccessMem32(pChipset,
                                        DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspRootAddr) + offsetof(struct _DSPIPCRootPointer, currentStatus),
                                        1, MEM_READ, &value)
                                    != 0) {
                ...
                if (retValue == 0) {
                        ...
                        if (dra7xxAccessMem32(pChipset,
                                DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspRootAddr) + offsetof(struct _DSPIPCRootPointer, rootAdminAddr),
                                1, MEM_READ, &pChipset->dspAdminStructAdr)
                            != 0) {
                                hb_slogf_error("%s: Access admin structure failed 3", __FUNCTION__);
                        }
                        ...
                        if (dra7xxSetConfigState(pChipset, DRA7XX_STARTED) != EOK) {
                                hb_slogf_error("%s: failed to call dra7xxSetConfigState", __FUNCTION__);
                        }
                } else {
                ...
                if (retValue == 0) {
                        ...
                        if (dra7xxSetConfigState(pChipset, DRA7XX_STARTED) != EOK) {
                                hb_slogf_error("%s: failed to call dra44xaSetConfigState", __FUNCTION__);
                        }
                } else {
                        hb_slogf_error("%s: No communication established", __FUNCTION__);
                        hb_slogf_error("%s: Read 0x%08X instead magic word 2", __FUNCTION__, value);
                }
...
int dspipc_dra7xx_startCommunication(const deviceTable_t* pDevice)
{
        ...
        if (dspipc_getNewMessageId((deviceTable_t*)pDevice, &msg.msgId) == 0) {
                ...
                hb_slogf_error("%s: begin to send version msg out", __FUNCTION__);
                if (dra7xxSendMsg(pChipset, &msg, 0, DRA7XX_SEND_DO_INT_TO_DSP) == 0) {
        ...
        if (retValue != 0) {
                dra7xx_mailbox_queue_status(pChipset);
        }
...
/**
 *  This function close the dra7xx dsp chipset layer
 *
 *  @param pDevice Pointer to the device table structure
 *  @return int 0 if everything was ok, -1 if an error has occurred
 */
int dspipc_dra7xx_close(deviceTable_t* pDevice)
{
        ...
        if (pDevice != NULL) {
          ...
          * */
                        ...
                        numFail++;
                        if ((numFail == 2) && (irqMailboxNum > 0)) {
                                dra7xx_mailboxExitDrv();
                                numFail = 0;
                                irqMailboxNum = 0;
                        }
...
/**
 *  This function is the getDspSwVers callout for the dra7xx dsp chipset layer
 *
 *  @param pDevice Pointer to the device table structure
...
int dspipc_dra7xx_resetState(const deviceTable_t* pDevice, resetType_t type)
{
        ...
        if (type == PREPARE_RESET) {
                ...
                delayRemain = 3;
                mdelay(delayRemain);
                if (dra7xxSetConfigState(pChipset, DRA7XX_NOT_INITIALIZED) != EOK) {
                        ...
                        if (typeOk) {
                                ...
                                if (((pMsg->msgSize > COMMANDMSG_SIZE) && (queueMsgType == DSPIPC_MSGTYPE_COMMAND)) || (pMsg->msgSize > 272)) {
                                        ...
                                        * */
                                        ...
                                        retValue = dra7xxSendMsg(pChipset, pMsg, pMsg_Phys, DRA7XX_SEND_DO_INT_TO_DSP);
                                }
                        } else {
...
static irqreturn_t mbx_irq_handler(int irq, void* dev_id)
{
        ...
        if (pDevice && pDevice->pChipset != NULL) {
                pChipset = (dspIpcChipset_t*)pDevice->pChipset;
                if (strcmp(pDevice->pDeviceName, "dsp") == 0) {
                        /* Send interrupt to thread for further handle. */
                        do {
                                /* Read interrupt status bit */
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(0), &value); /* Read IRQenable Reg 0 */
                                cnt++;
                        } while ((cnt < 10) && ((value & 0x00000001) == 0));

                        if ((value & 0x00000001) == 0x1) {
                                /* Read Num MSG's */
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(0), &msgNum); /* Msg Status */
                                while (msgNum > 0) {
                                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(0), &value); /* Read Msg Reg 0 */
                                        msgNum--;
                                }
                                if (value != 0xBEEFDEAD) {
...
out:
        ...
        dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(0), 0x00000001);
        return IRQ_HANDLED;
}
        ...
        if (pDevice && pDevice->pChipset != NULL) {
                ...
                if (strcmp(pDevice->pDeviceName, "m3") == 0) {
                        ...
                        if ((value & 0x100) == 0x100) {
                                /* Read Num MSG's */
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(4), &msgNum); /* Msg Status */
                                while (msgNum > 0) {
                                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(4), &value); /* Read Msg Reg 0 */
                                        msgNum--;
                                }
                                if (value != 0xBEEFDEAD) {
                                        hb_slogf_error("%s: Invalid Msg: 0x%x", __FUNCTION__, value);
...
out:
        /* clear interrupt status bit. */
        dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(0), 0x00000100);
        return IRQ_HANDLED;
}
                        ...
                        if (retValue != -1 && pDevice->intChId != -1) {
                                ...
                                if (pDevice->intCoId != -1) {
                                        if (pDevice->intNumber != -1) {
                                                ...
                                                hb_slogf_error("%s: device name:%s, int number:%d", __FUNCTION__, pDevice->pDeviceName, pDevice->intNumber);
                                                if (strcmp(pDevice->pDeviceName, "dsp") == 0) {

                                                        retValue = request_irq(pDevice->intNumber,
                                                            mbx_irq_handler,
                                                            IRQF_SHARED, /* in later 2.6 kernel version, the flags are changed from SA_* to IRQF_* */
                                                            "dspipc_irq",
                                                            (void*)pDevice);
                                                } else if (strcmp(pDevice->pDeviceName, "m3") == 0) {
                        ...
                        while ((pDevice->doLoop)) {
                                ...
                                if ((readnum = MsgReceive(pDevice->intChId, &intrCode, 1, (unsigned int)-1)) > 0) {
                                        ...
                                        case INTTHREAD_INT: /* Real Interrupt */
                                                ...
                                                if (pChipset->configState != DRA7XX_NOT_INITIALIZED) {
                                                        /* check asserting marker for DSP exception */
                                                        if (dra7xxAccessMem32(pChipset,
                                                                DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspAdminStructAdr) + ADMIN_OFFSET(dspAssertLine),
                                                        ...
                                                        if (dspAssertLine != 0) {
                                                                if (dra7xxAccessMem32(pChipset,
                                                                        DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspAdminStructAdr) + ADMIN_OFFSET(dspAssertFile),
                                                                ...
                                                                i = 0;
                                                                while (DSPIPCdra7xxFileNameArray[i].name != NULL) {
                                                                        if (DSPIPCdra7xxFileNameArray[i].num == dspAssertFile) {
                                                                                hb_slogf_error("%s: DSP assertion found in file %s at line %d",
                                                                                    __FUNCTION__, DSPIPCdra7xxFileNameArray[i].name, dspAssertLine);
                                                                                break;
                                                                        }
                                                                        i++;
                                                                }
                                                                if (DSPIPCdra7xxFileNameArray[i].name == NULL) {
                                                        ...
                                                        dspTxQueueAdr = dra7xx_get_queue_index(pChipset,
                                                        ...
                                                        while (numElements > 0) {
                                                                ...
                                                                if (pChipset->verbose > 3) {
                                                                        dra7xxDumpMessage("received", pMsg);
                                                                }
#endif
                                                                /* process message */
                                                                ...
                                                                (void)dra7xx_set_queue_index(pChipset,
                                                                    ADMIN_OFFSET(dspTxBufferAddr),
                                                ...
                                                if ((errCode = dra7xxInterruptUnmask(pDevice)) != EOK) {
 ...
 *  @return void
 */
static void dra7xxRegWrite32(const dspIpcChipset_t* pChipset, uint32_t RegModSel, uint32_t addr, uint32_t val)
{
        if (pChipset != NULL) {
                if (RegModSel == MOD_DEVCONF_REG) {
                        /* Module Conf Register */
                        iowrite32(val, (void __iomem*)((char __iomem*)pChipset->regModConf + addr));
                        /**((volatile unsigned long*)((char*)pChipset->regModConf + addr)) = val; */
#ifdef VERBOSE_ENABLE
                        if (pChipset->verbose > 3) {
 ...
 */
static int dra7xxAccessMem32_DMA(dspIpcChipset_t* pChipset,
    uint32_t dps_addr,
    uint32_t numLongWords,
    memAccessDir_t dir,
        ...
        } else /* do transfer on Overlapping Pages */
        {
                ...
                if (dir == MEM_READ) {
                        while (numLongWords != 0) {
                                if (dra7xxMemRead(pChipset, dps_addr, &value) != 0) {
                                        hb_slogf_error(DRA7XXREAD_FAILED, __FUNCTION__);
                                }
                                *pVal++ = value;
                                dps_addr += 4;
                ...
                } else /* MEM_WRITE */
                {
                        ...
                        while (numLongWords != 0) {
                                value = *pVal++;
                                if (dra7xxMemWrite(pChipset, dps_addr, value) != 0) {
                                        hb_slogf_error(DRA7XXMEMWRITE_FAILED_2, __FUNCTION__);
                                }
                                dps_addr += 4;
                ...
                if (end_off == 0) {
                        ...
                        if (file_address != 0) {
                                ...
                                if ((ret_val = read(bootFd, (void*)file_address, end_off)) != end_off) {
                                        ...
                                        if (entryPoint == 0) {
                                                hb_slogf_error("%s: error load_elf_file() failed (%d)", __FUNCTION__, entryPoint);
                                        } else {
                                                if (entryPoint & 0x000003FF) {
                                                        ...
                                                        hb_slogf_error("%s: load_elf_file() done: DSP-StartAddr: 0x%X", __FUNCTION__, entryPoint);
                                                }
                                        }

                                        /* Set Startaddress of DSP (Internal L2-RAM of DSP) */
                                        /*dsp boot address have to be 1024(2^10) align, and be writen to [0:21] bit of 	the register	 */
                                        entryPoint = ((entryPoint & 0xFFFFFC00) >> 10) | 0x2000000;

                                        dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, DCM_DSPBOOTADDR, entryPoint);
                                        dra7xxRegWrite32(pChipset, MOD_STATCTRL_REG, DCM_DSPBOOTADDR, entryPoint);
                                        dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, DCM_DSPBOOTADDR, &mem_val);
#ifdef VERBOSE_ENABLE
                                        if (pChipset->verbose) {
                                                hb_slogf_error("%s: read: DSP-BOOTADDR: 0x%x", __FUNCTION__, mem_val);
                                        }
                                        ...
                                        do {
                                                mdelay(1);
                                                count++;
                                                dra7xxRegRead32(pChipset, MOD_STATCTRL_REG, CM_DSP_CLKCTRL, &mem_val); /* Read RM_DSP_RSTST Register */
                                        } while (((mem_val & 0x30000) != 0x0) && (count < 100));

                                        if ((mem_val & 0x30000) != 0x0) {
                                                hb_slogf_error("%s: DSP module mode is not actived !!!", __FUNCTION__);
                                        } else {
                                                hb_slogf_error("%s: DSP module mode is actived !!!", __FUNCTION__);
                                        }
...
static int dra7xxMemWrite(const dspIpcChipset_t* pChipset, const uint32_t addr, const uint32_t value)
{
        ...
        if ((pChipset != NULL) && (pChipset->memAccAddr != NULL)) {
                if ((addr & 0x03U) == 0) {
                        *((volatile unsigned long __force*)((char __iomem*)(pChipset->memAccAddr) + addr_masked)) = value;
                } else {
...
static int dra7xxMemRead(const dspIpcChipset_t* pChipset, const uint32_t addr, volatile uint32_t* dest)
{
        ...
        if ((pChipset != NULL) && (pChipset->memAccAddr != NULL)) {
                if ((addr & 0x03U) == 0) {
                        if (dest != NULL) {
                                value = *((volatile unsigned long __force*)((char __iomem*)(pChipset->memAccAddr) + addr_masked));
                                *dest = value;
                        } else {
 ...
 *  @return void
 */
static void dra7xxMemRead_oc(const dspIpcChipset_t* pChipset,
    uint32_t addr, /* addr */
    uint32_t* dest,
    uint32_t numLongWords)
{
 ...
 *  @return void
 */
static void dra7xxMemWrite_oc(const dspIpcChipset_t* pChipset,
    uint32_t addr, /* addr */
    ...
    uint32_t numLongWords)
{
        ...
        while (numLongWords != 0) {
                *((volatile unsigned long*)((char*)((uint32_t __force)pChipset->memAccAddr + addr_masked))) = ENDIAN_LE32(*value);
                value++;
                addr_masked += 4;
                numLongWords--;
        }
    ...
    uint32_t* pVal)
{
        ...
        {
                if (dir == MEM_READ) {
                        dra7xxMemRead_oc(pChipset, addr, pVal, numLongWords);
                } else {
                        dra7xxMemWrite_oc(pChipset, addr, pVal, numLongWords);
                }
        } else {
                if (dir == MEM_READ) {
                        while (numLongWords != 0) {
                                if (dra7xxMemRead(pChipset, addr, &value) != 0) {
                                        hb_slogf_error(DRA7XXREAD_FAILED, __FUNCTION__);
                                }
                                *pVal++ = value;
                                addr += 4;
                        ...
                        while (numLongWords != 0) {
                                value = *pVal++;
                                if (dra7xxMemWrite(pChipset, addr, value) != 0) {
                                        hb_slogf_error(DRA7XXMEMWRITE_FAILED, __FUNCTION__);
                                }
                                addr += 4;
    ...
    const int32_t* pRead, const int32_t* pWrite)
{
        ...
        accAdr = DRA7XX_CONV_DSP2HOST(pChipset, pChipset->dspAdminStructAdr) + queueOff;
        if (setMemSelAdr(pChipset, accAdr) != 0) /* Set to Mapped RAM */
        {
                hb_slogf_error("%s: setting Mem-Select", __FUNCTION__);
        }

        if (dra7xxMemRead(pChipset, accAdr, &value) != 0) {
                hb_slogf_error(DRA7XXMEMREAD_FAILED, __FUNCTION__, (-4));
        }
        dspQueueAdr = DRA7XX_CONV_DSP2HOST(pChipset, value);
        ...
        if (pWrite != NULL) {
                ...
                if (dra7xxMemWrite(pChipset, accAdr, *pWrite) != 0) {
                        hb_slogf_error(DRA7XXMEMWRITE_FAILED_2, __FUNCTION__, (-1)); /* DRA7XXMEMWRITE_FAILED_2  DM642MEMWRITE_FAILED_2 */
                }
        }
        ...
        if (pRead != NULL) {
                ...
                if (dra7xxMemWrite(pChipset, accAdr, *pRead) != 0) {
                        hb_slogf_error(DRA7XXMEMWRITE_FAILED_2, __FUNCTION__, (-2));
                }
        }
    ...
    int32_t* pRead, int32_t* pWrite)
{
        ...
        if (dra7xxMemRead(pChipset, accAdr, &value) != 0) {
                hb_slogf_error(DRA7XXMEMREAD_FAILED, __FUNCTION__, (-1)); /* DRA7XXMEMREAD_FAILED */
        }
        dspQueueAdr = DRA7XX_CONV_DSP2HOST(pChipset, value);
        ...
        if ((dspQueueAdr >= pChipset->dspRootAddr) && (dspQueueAdr < pChipset->dspRootAddr + pChipset->MemSize)) {
                ...
                if (pWrite != NULL) {
                        ...
                        if (dra7xxMemRead(pChipset, accAdr, (uint32_t*)pWrite) != 0) {
                                hb_slogf_error(DRA7XXMEMREAD_FAILED, __FUNCTION__, (-2)); /* DRA7XXMEMREAD_FAILED */
                        }
                }
                ...
                if (pRead != NULL) {
                        ...
                        if (dra7xxMemRead(pChipset, accAdr, (uint32_t*)pRead) != 0) {
                                hb_slogf_error(DRA7XXMEMREAD_FAILED, __FUNCTION__, (-3));
                        }
                }
 ...
 *  @param setInterrupt Signalize the packet with an interrupt (DRA7XX_SEND_NO_INT_TO_DSP|DRA7XX_SEND_DO_INT_TO_DSP)
    ...
    const dra7xxSendIndicator_t setInterrupt)
{
        ...
        dspRxQueueAdr = dra7xx_get_queue_index(pChipset,
        ...
        if (numElements >= COMMANDQUEUE_LENGTH) {
                ...
                if (setInterrupt == DRA7XX_SEND_DO_INT_TO_DSP) {
...
#ifdef VERBOSE_ENABLE
        if (pChipset->verbose > 3) {
                dra7xxDumpMessage("before send call", pMsg);
        }
#endif
        ...
        if (pPhysMsg == 0) {
                if (dra7xxAccessMem32(pChipset,
                        ...
                        sizeof(DSPIPCMessageQueueEntry) >> 2, MEM_WRITE, (uint32_t*)pMsg)
                    != 0) {
                        hb_slogf_error("%s: Access failed", __FUNCTION__);
                }
        } else {
                if (dra7xxAccessMem32_DMA(pChipset,
                        dspRxQueueAdr + offsetof(struct _DSPIPCMessageQueue, msg) + (((uint32_t)dspRxQueueWriteIndex) % COMMANDQUEUE_LENGTH) * sizeof(DSPIPCMessageQueueEntry),
        ...
        if (pChipset != NULL) {
                ...
                if (pChipset->verbose > 0) {
                        if (strcmp(pChipset->pDevice->pDeviceName, "dsp") == 0) {
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(1), &mem_val); /* Read Fifo-Status Register of Mailbox 1 */
                        } else if (strcmp(pChipset->pDevice->pDeviceName, "m3") == 0) {
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(5), &mem_val); /* Read Fifo-Status Register of Mailbox 5 */
                        }
                ...
                if (strcmp(pChipset->pDevice->pDeviceName, "dsp") == 0) {
                        do {
                                ...
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(1), &value); /* Read Fifo-Status Register of Mailbox 1 */
                        ...
                        /* get DSP interrupt status */
                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(1), &value);
                        if (value & 0x00000004) /*interrupt is set to DSP */
                        {
                                ...
                                do {
                                        ...
                                        counter++;
                                        /* get DSP interrupt status */
                                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(1), &value); /* Read Fifo-Status Register of Mailbox 1 */
                                ...
                                dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(1), 0xDEADBEEF); /* write message  */
                        }
                } else if (strcmp(pChipset->pDevice->pDeviceName, "m3") == 0) {
                        do {
                                counter++;
                                value_2 = MAILBOX_FIFOSTATUS(5);
                                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(5), &value); /* Read Fifo-Status Register of Mailbox 1 */
                        } while ((value == 0x00000001) && (counter < 1000));
                        ...
                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(2), &value);
                        if (value & 0x00000400) /*interrupt is set to M3 */
                        {
                                ...
                                do {
                                        ...
                                        counter++;
                                        /* get DSP interrupt status */
                                        dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(2), &value); /* Read Fifo-Status Register of Mailbox 1 */
                                ...
                                dra7xxRegWrite32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(5), 0xDEADBEEF); /* write message     */
                        }
                }
                                                                ...
                                                                for (i = 0; i < pRecvStreamMsg->hdr.numEntries; i++) {
                                                                        ...
                                                                        if (pMemObj != NULL) {
                                                                                ...
                                                                                if (pChipset->verbose > 3) {
                                                                                        if (pStreamMsg->bufferFlags & SMF_PROVIDE_BUFFER_RET) {
#ifdef DUMP_DATA
                                                                                                dra7xxDumpData("(DATA RECEIVE)", (void*)(((char*)(pMemObj->pVirtAddr) + memOffset)), pStreamMsg->ranges[i].dataSize);
#endif
                                                                                        }
                                                                                }
#endif
...
static int ProcessInternalAdmin(dspIpcChipset_t* pChipset, DSPIPCMessageQueueEntry* pMsg)
{
        ...
        pAdminMsg = &(pMsg->payload.adminMsg);
        if ((pChipset->configState == DRA7XX_VERSION_MATCH) || (pAdminMsg->generic.cmdType == DCCMID_VERSION)) {
                switch (pAdminMsg->generic.cmdType) {
                case DCCMID_INVALID:
                ...
                case DCCMID_VERSION:
...
#ifdef VERBOSE_ENABLE
                        if (pChipset->verbose > 0) {
                                hb_slogf_error("%s: Version from %s received <%d.%d.%d>", __FUNCTION__, pChipset->pDevice->pDeviceName,
                        ...
                        if (matched == 0) {
                                hb_slogf_error("%s: Version mismatch (Driver:%d.%d.%d DSP:%d.%d.%d)",
                                    ...
                                    pVersion->swMajor, pVersion->swMinor, pVersion->swBugfix);
                                if (dra7xxSetConfigState(pChipset, DRA7XX_VERSION_MISMATCH) != EOK) {
                                        hb_slogf_error("%s: failed to call dra7xxSetConfigState", __FUNCTION__);
                                }
                        } else {
                                if (dra7xxSetConfigState(pChipset, DRA7XX_VERSION_MATCH) != EOK) {
                                        hb_slogf_error("%s: failed to call dra7xxSetConfigState", __FUNCTION__);
                                }
                        }
#ifdef DUMP_DATA
                        dra7xxDumpData("(DSP VERSION)", (void*)((char*)(&(pAdminMsg->content.version))), sizeof(struct _DSPIPCVersionMsg));
#endif
...
static int dra7xx_mailbox_queue_status(dspIpcChipset_t* pChipset)
{
        ...
        if (strcmp(pChipset->pDevice->pDeviceName, "dsp") == 0) {
                ...
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(0), &mb_int);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(0), &mb_irq);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(0), &mb_fifo);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(0), &mb_num);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(0), &mb_msg);
                ...
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(1), &mb_int);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(1), &mb_irq);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(1), &mb_fifo);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(1), &mb_num);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(1), &mb_msg);
                ...
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(0), &mb_int);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(0), &mb_irq);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(4), &mb_fifo);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(4), &mb_num);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(4), &mb_msg);
                ...
                /*get mailbox interrupt status/mailbox num/mailbox message which is used from Acore to DSP */
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQENABLE_SET_REG(2), &mb_int);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_IRQSTATUS_CLR_REG(2), &mb_irq);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_FIFOSTATUS(5), &mb_fifo);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MSGSTATUS(5), &mb_num);
                dra7xxRegRead32(pChipset, MOD_DEVCONF_REG, MAILBOX_MESSAGE(5), &mb_msg);
        ...
        while (numElements > 0) {
                ...
                /*dump message */
                dra7xxDumpMessage("received", &msg);
                dspTxQueueReadIndex++;
                numElements--;
        }
