...
enum {
        ...
        DEV_CONF_MODULE = 0x48800000U,          /*mailbox base address */
        DEV_CONF_MODULE_MEM_SIZE = 0x00066000U, /*bytes length to ioremap the register */

        DSP_IDLE_CFG_REG = 0x0014061CU, /* MB to clude the Mailbox Register   Memory for Device Configuaration Module                   */

        MOD_STAT_CRTL_BASE = 0x4A000000U,
        MOD_STAT_CRTL_MEM_SIZE = 0x00F00000U,

        PM_DSP_PWRSTCTRL = 0x00E06400U, /* RW 32 0x0003 0000 0x00 0x0A00 */
        PM_DSP_PWRSTST = 0x00E06404U,   /* RO 32 0x0000 0017 0x04 0x0A04 */
        RM_DSP_RSTCTRL = 0x00E06410U,   /* RW 32 0x0000 0003 0x10 0x0A10 */
        RM_DSP_RSTST = 0x00E06414U,     /* RW 32 0x0000 0000 0x14 0x0A14 */

        CM_DSP_CLKSTCTRL = 0x00005400U, /* RW 32 0x0000 0001 0x00 0x0400 */
        CM_DSP_CLKCTRL = 0x000005420U,  /* RW 32 0x0007 0000 0x20 0x0420 */
};

/*for J6*/
#define CTRL_CORE_MPU_IRQ_8_9 0x00002A4CU
#define IRQ_XBAR_MBOX_2_USR_0 237

#define CTRL_CORE_MMR_LOCK_2 (0x00002544U)

#define CM_ALWON_MAILBOX_CLKC (0x00008d48U) /* CM_ALWON_MAILBOX_CLKC */
#define CM_CLKOUT_CTRL (0x00000100U)        /* CM_CLKOUT_CTRL */

#define DCM_DSPBOOTADDR (0x0000255CU) /*CTRL_CORE_CONTROL_DSP1_RST_VECT */

#define CTRL_MAILBOX_BASE (0x4A000000U)
 ...
 */
enum {
        MAILBOX_REVISION = 0x0003A000U,  /* Mailbox Revision */
        MAILBOX_SYSCONFIG = 0x0003A010U, /* Mailbox System Configuration */

        MAILBOX_MESSAGE_m = 0x0003A040U,    /* Mailbox Message Register      0x0000 0040 + (0x4 * m) */
        MAILBOX_FIFOSTATUS_m = 0x0003A080U, /* Mailbox FIFO Status Register     0x0000 0080 + (0x4 * m) */
        MAILBOX_MSGSTATUS_m = 0x0003A0C0U,  /* Mailbox Message Status Register    0x0000 00C0 + (0x4 * m) */

        MAILBOX_IRQSTATUS_RAW_u = 0x0003A100U, /* Mailbox IRQ RAW Status Register    0x0000 0100 + (0x10 * u) */
        MAILBOX_IRQSTATUS_CLR_u = 0x0003A104U, /* Mailbox IRQ Clear Status Register    0x0000 0104 + (0x10 * u) */
        MAILBOX_IRQENABLE_SET_u = 0x0003A108U, /* Mailbox IRQ Enable Set Register      0x0000 0108 + (0x10 * u) */
        MAILBOX_IRQENABLE_CLR_u = 0x0003A10CU  /* Mailbox IRQ Enable Clear Register    0x0000 010C + (0x10 *u) */
};

#define MAILBOX_MESSAGE(m) (0x0003A040U + (0x4 * m))        /* Mailbox Message Register           0x0000 0040 + (0x4 * m) */
#define MAILBOX_FIFOSTATUS(m) (0x0003A080U + (0x4 * m))     /* Mailbox FIFO Status Register       0x0000 0080 + (0x4 * m) */
#define MAILBOX_MSGSTATUS(m) (0x0003A0C0U + (0x4 * m))      /* Mailbox Message Status Register    0x0000 00C0 + (0x4 * m) */
#define MAILBOX_IRQSTATUS_RAW(u) (0x0003A100U + (0x10 * u)) /* Mailbox IRQ RAW Status Register    0x0000 0100 + (0x10 * u) */

#define MAILBOX_IRQSTATUS_CLR_REG(u) (0x0003A104U + (0x10 * u))  /*MAILBOX_IRQENABLE_SET_u (2) RW Mailbox IRQ Status Clear Register 0x0000 0104 + (0x10 * u) */
#define MAILBOX_IRQENABLE_SET_REG(u) (0x0003A108U + (0x10 * u))  /*MAILBOX_IRQENABLE_SET_u (2) RW Mailbox IRQ Enable Set Register 0x0000 0108 + (0x10 * u) */
#define MAILBOX_IRQENABLE_CLR_REG(u) (0x0003A10CU + (0x10 * u))  /*MAILBOX_IRQENABLE_SET_u (2) RW Mailbox IRQ Clear Register 0x0000 010C + (0x10 * u) */
