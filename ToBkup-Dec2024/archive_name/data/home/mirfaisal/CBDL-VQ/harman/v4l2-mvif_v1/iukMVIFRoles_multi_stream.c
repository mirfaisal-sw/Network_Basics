...
static int32_t createTVTunerMVIF( TVideoIFaceCommon *pMVIFCommon, uint32_t memAddr, uint32_t mvifIndex )
{
	...
	mvif_Set_streamIdTalker( pMVIFCommon, 0, 0x91E0F000, 0xFE000006 );
       //mvif_Set_StreamStateTalker( pMVIFCommon, 0 , MVIF_SST_TALKING);
#if !defined (CONFIG_NTG7_CSB) && !defined (CONFIG_NTG6_CSB)
       mvif_Set_StreamStateTalker( pMVIFCommon, 0 , MVIF_SST_IDLE);
#else
	mvif_Set_StreamStateTalker( pMVIFCommon, 0 , MVIF_SST_TALKING);

	if ( 0 == mvif_Create_Talker( pMVIFCommon, 1, 188 ) ) {
		return 0;
	}

	//TODO: these settings have to come from Audio /Video Management Application, just a default
	mvif_Set_destMacAddressTalker( pMVIFCommon, 1, 0x91E0, 0xF000FE11 );
	mvif_Set_streamIdTalker( pMVIFCommon, 1, 0x91E0F000, 0xFE100211 );
	mvif_Set_StreamStateTalker( pMVIFCommon, 1 , MVIF_SST_TALKING);
#endif
 ...
 * @return 0 init not done; >= 0 no error
 */
uint32_t getMVIFFreeTsPtrEntriesTalker( uint32_t indx )
{
	if ( NULL != pMVIFCommon )
	{
