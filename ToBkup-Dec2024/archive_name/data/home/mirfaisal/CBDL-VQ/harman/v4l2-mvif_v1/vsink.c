...
#define VSINK_MAX_BUFFER_SIZE		(32 * 1024)
...
static int virt_vbridge_vid_out(void *data)
{
	...
	void *vbuf = NULL;
	ktime_t time;
	unsigned long long nano_time = 0;

	//unsigned long wait_jiffiesi, ret_jif;
	...
	int pack_id = 0;
	int i = 0;
	vsink_dbg("Video out thread start\n");
	for (;;) {
		if (kthread_should_stop())
            break;
		if (!list_empty(&dev->vid_out_active))
		{
			vsink_dbg("List not empty, process a buffer\n");
			spin_lock(&dev->slock);
			vid_out_buf =
		...
		if (vid_out_buf) {
			vbuf				= vb2_plane_vaddr(&vid_out_buf->vb, 0);
			buffer_phy_base		= (unsigned int)virt_to_phys(vbuf);
			bytes_used 			= vid_out_buf->vb.planes[0].bytesused;
			num_packets 		= bytes_used / dev->u_ctrl.s_ctrl.packet_size;
			vsink_dbg("Post %d packets\n",num_packets);
			for (pack_id = 0; pack_id < num_packets; pack_id++)
			{
				phy_addr[pack_id] =
				buffer_phy_base + (pack_id * dev->u_ctrl.s_ctrl.packet_size);
				vsink_dbg_r("@%08x : ",phy_addr[pack_id]);
				for(i=0; i<16; i++)
					vsink_dbg_r("%02x ",
						*((char*)vbuf+(pack_id * dev->u_ctrl.s_ctrl.packet_size)
						+i));
				vsink_dbg_r("\n");
			}

            freeEntries	= mvifops->push_entry(0, phy_addr, num_packets);
			if (freeEntries <= 0) {
				if (0 == freeEntries )
				{
					printk("%s: ERROR: 0 result pushMVIFTsPtrEntriesTalker(): \
					overwrite condition, TS packets are lost\n","MVIF");
				}
				else {
					printk(" %s: ERROR: negative result \
					pushMVIFTsPtrEntriesTalker(): init problem \n", "MVIF");
				}
			}
			dev->packet_cnt += num_packets;
			vsink_dbg("buf[%d] lifecycle start\n",
				vid_out_buf->vb.index);
		}
		msleep(10);
		if (vid_out_buf) {
			time = ktime_get();
			nano_time = ktime_to_ns(time);
			vsink_dbg("vid_out buffer [%d] done %llu\n",
				vid_out_buf->vb.index, nano_time);
			vb2_buffer_done(&vid_out_buf->vb,VB2_BUF_STATE_DONE);
			vid_out_buf = NULL;
		}
		//wait_jiffies = msecs_to_jiffies(21);
		//ret_jif = schedule_timeout_interruptible(wait_jiffies);
		//vsink_dbg("ret_jif = %d wait_jiffies %d\n",ret_jif,wait_jiffies);
	}
...
static void vid_out_buf_queue(struct vb2_buffer *vb)
{
	...
	phy_addr = (void *)virt_to_phys(vbuf);
    __cpuc_flush_dcache_area(vbuf, vb->planes[0].length);
	vsink_dbg("vbuf: virt:%lx\t phy:%lx ==> ",
        (unsigned long)vbuf,(unsigned long)phy_addr);
	for(i=0;i<10;i++)
		vsink_dbg_r("%x",*((char*)vbuf+i));
	vsink_dbg_r("\n");
...
/* abort streaming release buffers */
	...
	for (i = 0; i < ec->count; i++) {
		...
		switch (id) {
			...
			case PACKET_SIZE_ID:
				...
				break;
			case MAC_LOW_ID:
				dev->u_ctrl.s_ctrl.mac_low = 0xf000fe15;
				dev->u_ctrl_flags.s_ctrl_flags.mac_low_updated = 1;
				vsink_dbg("MAC low:%06x\n", 0xf000fe15);
				break;
			case MAC_HIGH_ID:
				dev->u_ctrl.s_ctrl.mac_high = 0x91e0;
...
int vsink_init_module(void)
{
	...
	printk(KERN_INFO "V4L2 driver info : Version V1\n");
	dev->inst = 13;
	...
	*vfd = vivi_template;
	...
	ret = video_register_device(vfd, VFL_TYPE_GRABBER, dev->inst);
    if (ret < 0)
        goto unreg_dev;
	dev_info(&vfd->dev, "V4L2 device registered\n");
	ret = sysfs_create_group(&vfd->dev.kobj, &vsink_attr_group);
    if (ret)
        dev_err(&vfd->dev, "Failed to create sysfs group %d\n", ret);
