...
struct virtvid *dev;
//static struct timer_list v4l2_timer;
//static int  v4l2_log_delay = 1000;
...
/*
void  packet_analyze_log(unsigned long data) {

	printk(KERN_INFO "V4L2 : Input count = %llu\t output count = %llu\n",input_packet_count,output_packet_count);
...
static int virt_vbridge_vid_out(void *data)
{
	...
	vsink_dbg("v4l2 :~ Video out thread end\n");
	return 0;
}
		...
		struct v4l2_format *f)
{
	...
	{	number_of_entries = f->fmt.pix.sizeimage / dev->u_ctrl.s_ctrl.packet_size;
