 ...
 */
int32_t mvif_Get_FreeTsPtrEntriesTalker(TVideoIFaceCommon *pCommon, uint32_t indx )
{
	TVideoIFaceStream *pStream;
	int32_t wrIdx = 0;
	int32_t rdIdx = 0;
	...
	else
	{
		if(wrIdx > rdIdx )
		{
			return (pStream->numVideoBuffer - ( wrIdx - rdIdx ) );
		}
		else
		{
			return ( rdIdx - wrIdx );
...
static int32_t PushVideoTs( TVideoIFaceStream *pStream, uint32_t * addrvideobufStart, uint32_t * videoAddresses, int pushts)
{
   ...
   if(pushts>0)
   {
     ...
     indicate that there is overwriting happening but we continue writing.*/
       ...
       {
		if((getMVIFFreeTsPtrEntriesTalker(0)) < (pushts-i)) {
    	 		pStream->overwrite++;
         		retval = -1;
		}
       }
