...
#include <soc/samsung/acpm_mfd_spmi.h>
#include <soc/samsung/acpm_ipc_ctrl.h>

#if IS_ENABLED(CONFIG_EXYNOS_ACPM)
int exynos_spmi_read_reg(struct device_node *acpm_mfd_node, u8 sid, u16 type, u8 reg, u8 *dest)
{
	unsigned int channel_num = 0, size = 0;
	...
	if (!acpm_ipc_request_channel(acpm_mfd_node, NULL, &channel_num, &size)) {
		...
		*dest = read_protocol(config.cmd[1], DEST);
		...
		if (ret) {
			pr_err(ACPM_MFD_PREFIX "%s - APM's spmi_rx fail.\n", __func__);
			return ret;
		}
...
EXPORT_SYMBOL_GPL(exynos_spmi_read_reg);

int exynos_spmi_bulk_read(struct device_node *acpm_mfd_node, u8 sid, u16 type, u8 reg, int count, u8 *buf)
{
	unsigned int channel_num = 0, size = 0;
	struct ipc_config config;
	unsigned int command[4] = {0,};
...
EXPORT_SYMBOL_GPL(exynos_spmi_bulk_read);

int exynos_spmi_write_reg(struct device_node *acpm_mfd_node, u8 sid, u16 type, u8 reg, u8 value)
{
	unsigned int channel_num = 0, size = 0;
	struct ipc_config config;
	unsigned int command[4] = {0,};
	...
	if (!acpm_ipc_request_channel(acpm_mfd_node, NULL, &channel_num, &size)) {
		...
		ret = read_protocol(config.cmd[1], RETURN);
		if (ret) {
			pr_err(ACPM_MFD_PREFIX "%s - APM's spmi_tx fail.\n", __func__);
			return ret;
		}
...
EXPORT_SYMBOL_GPL(exynos_spmi_write_reg);

int exynos_spmi_bulk_write(struct device_node *acpm_mfd_node, u8 sid, u16 type, u8 reg, int count, u8 *buf)
{
	unsigned int channel_num = 0, size = 0;
	struct ipc_config config;
	unsigned int command[4] = {0,};
...
EXPORT_SYMBOL_GPL(exynos_spmi_bulk_write);

int exynos_spmi_update_reg(struct device_node *acpm_mfd_node, u8 sid, u16 type, u8 reg, u8 value, u8 mask)
{
	unsigned int channel_num = 0, size = 0;
	struct ipc_config config;
	unsigned int command[4] = {0,};
...
EXPORT_SYMBOL_GPL(exynos_spmi_update_reg);
#endif
