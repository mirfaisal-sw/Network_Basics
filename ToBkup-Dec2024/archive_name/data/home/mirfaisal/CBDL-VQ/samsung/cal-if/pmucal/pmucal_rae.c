...
struct pmucal_rae_variant *pmucal_rae_variant_model;

struct pmucal_rae_variant pmucal_rae_variant_v9 = {
	.p_pmucal_p2v_list = pmucal_p2v_list_v9,
	.p_pmucal_p2v_list_size = &pmucal_p2v_list_size_v9,
};

struct pmucal_rae_variant pmucal_rae_variant_v920_evt0 = {
	.p_pmucal_p2v_list = pmucal_p2v_list_v920_evt0,
	.p_pmucal_p2v_list_size = &pmucal_p2v_list_size_v920_evt0,
};

struct pmucal_rae_variant pmucal_rae_variant_v920_evt2 = {
	.p_pmucal_p2v_list = pmucal_p2v_list,
	.p_pmucal_p2v_list_size = &pmucal_p2v_list_size,
};

struct pmucal_rae_variant pmucal_rae_variant_v620_evt0 = {
	.p_pmucal_p2v_list = pmucal_p2v_list_v620_evt0,
	.p_pmucal_p2v_list_size = &pmucal_p2v_list_size_v620_evt0,
};
/**
	...
	for (i = 0; i < seq_size; i++) {
		...
		if (seq[i].base_pa) {
			for (j = 0; j < *pmucal_rae_variant_model->p_pmucal_p2v_list_size; j++)
				if (pmucal_rae_variant_model->p_pmucal_p2v_list[j].pa == (phys_addr_t)seq[i].base_pa)
					break;
			...
			if (j == *pmucal_rae_variant_model->p_pmucal_p2v_list_size) {
				...
				seq[i].base_va = pmucal_rae_variant_model->p_pmucal_p2v_list[j].va;
			}
		}
		if (seq[i].cond_base_pa) {
			for (j = 0; j < *pmucal_rae_variant_model->p_pmucal_p2v_list_size; j++)
				if (pmucal_rae_variant_model->p_pmucal_p2v_list[j].pa == (phys_addr_t)seq[i].cond_base_pa)
					break;
			...
			if (j == *pmucal_rae_variant_model->p_pmucal_p2v_list_size) {
				...
				seq[i].cond_base_va = pmucal_rae_variant_model->p_pmucal_p2v_list[j].va;
			}
		}
	}
 ...
 * @prdcode
 * @unit_name{PMU_CAL}
 * @purpose "To get virtual address for all physical address in PMU_CAL"
 * @logic "ioremap the address for all address list and store it to the structure"
 * @noparam
 * @retval{-, -, -, >=0, <0}
 */
int pmucal_rae_init(void)
{
	...
	if (!(*pmucal_rae_variant_model->p_pmucal_p2v_list_size)) {
		pr_err("%s %s: there is no p2vmap. aborting init...\n",
				PMUCAL_PREFIX, __func__);
		return -ENOENT;
	}

	for (i = 0; i < *pmucal_rae_variant_model->p_pmucal_p2v_list_size; i++) {
#ifdef PWRCAL_TARGET_LINUX
		pmucal_rae_variant_model->p_pmucal_p2v_list[i].va = ioremap(pmucal_rae_variant_model->p_pmucal_p2v_list[i].pa, SZ_64K);
		if (pmucal_rae_variant_model->p_pmucal_p2v_list[i].va == NULL) {
	...
	pr_info("CAL : pmucal_rae_init done\n");
