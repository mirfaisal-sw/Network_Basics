...
static struct pll_spec gplla0535X_spec = {
	1,		63,
	64,		1023,
	0,		6,
	0,		0,
...
static struct pll_spec frd5311apll_spec = {
	1,		63,
	64,		1023,
	0,		6,
	0,		0,
...
static struct pll_spec frd5312apll_spec = {
	1,		63,
	64,		1023,
	0,		6,
	-32767,		32767,
...
struct pll_spec *pll_get_spec(struct cmucal_pll *pll)
{
	...
	case PLL_0821X:
		...
		break;
	case pll_a0535x:
		pll_spec = &gplla0535X_spec;
		break;
	case frd_5311_apll:
		pll_spec = &frd5311apll_spec;
		break;
	case frd_5312_apll:
		pll_spec = &frd5312apll_spec;
		break;
	default:
		pll_spec = NULL;
	}
				 ...
				 unsigned long long rate)
{
	...
	if (rate > ((unsigned long long)m << 24))
		rate -= ((unsigned long long)m << 24);
	else
		rate -= (((unsigned long long)m - 1) << 24);
...
{
	struct pll_spec *pll_spec;
	unsigned int p = 0, m = 0, s = 0, f = 0, m_temp = 0;
	int k = 0;
	unsigned long long fref = 0, fvco = 0, fout = 0;
	unsigned long long min_diff = 0, tmp = 0;
