 ...
 * @logic "setting target clock node rate"
 * @params
 ...
 * @logic "getting target clock node rate"
 * @params
 * @param{in, id, unsigned int, >=0}
 * @endparam
 * @retval{ret, int, -, >=0, <0}
 ...
 * @logic "enable target clock node"
 * @params
 * @param{in, id, unsigned int, >=0}
 * @endparam
 ...
 * @logic "disable target clock node"
 * @params
 * @param{in, id, unsigned int, >=0}
 * @endparam
 * @retval{ret, int, -22, >=0, <0}
...
unsigned int vclk_get_lv_num(unsigned int id)
{
	...
	if (!vclk) {
		pr_info("CAL interface : vclk is Null\n");
		goto err;
	}
	...
	pr_info("CAL interface : [%s] cal_dfs_get_lv_num : %d\n",
			vclk->name, lv_num);
err:
	return lv_num;
...
static int vclk_get_dfs_info(struct vclk *vclk)
{
	...
	gen_block = ect_get_block("GEN");
	if (gen_block) {
		if (sprintf(buf, "MINMAX_%s", vclk->name) < 0) {
		...
		if (minmax != NULL) {
			for (i = 0; i < minmax->num_of_row; i++) {
				...
				if (minmax_table[0] == id_get_asv_fusing())
					break;
			}
		}
	}
...
int vclk_initialize(void)
{
	...
	pr_info("vclk initialize for cmucal done\n");
