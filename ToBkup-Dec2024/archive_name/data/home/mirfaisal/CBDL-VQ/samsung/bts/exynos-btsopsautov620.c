...
static void init_trexbts(void __iomem *base)
{
	...
	w1 = w1 & ~(0x1 << ENABLE_TOKEN_FLOW_SHIFT);
	checkReadAfterWrite(w1, base + SCI_CTRL);

	checkReadAfterWrite(0x10101010, base + TH_IMM_R_0);
	checkReadAfterWrite(0x10101010, base + TH_IMM_W_0);
	checkReadAfterWrite(0x08080808, base + TH_HIGH_R_0);
	checkReadAfterWrite(0x08080808, base + TH_HIGH_W_0);
}
 ...
 ****************************************************************/
int register_btsautov620_ops(struct bts_info *info)
{
...
EXPORT_SYMBOL(register_btsautov620_ops);
