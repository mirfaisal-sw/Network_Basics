...
extern int register_btsautov9_ops(struct bts_info *info);
extern int register_btsautov920_ops(struct bts_info *info);
extern int register_btsautov620_ops(struct bts_info *info);
extern int exynos_bts_debugfs_init(void);

#define NOCL0_USER_REG4	((void *)0x1DC20010)
#define checkReadAfterWrite(w, addr) (\
{ \
	__raw_writel(w, addr); \
	if (w != __raw_readl(addr)) \
		__raw_writel(0x1, NOCL0_USER_REG4); \
})
