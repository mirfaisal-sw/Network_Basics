...
#define BTSDBG_LOG(x...)	do { if (btsdbg_log)	dev_notice(x); } while (0)
...
static int exynos_bts_hwstatus_open_show(struct seq_file *buf, void *d)
{
	...
	memset(&stat, 0, sizeof(struct bts_stat));
	/* Check BTS setting */
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if (info.ops->get_bts != NULL && info.type == IP_BTS) {
			...
			if (!ret) {
				seq_printf(buf, "%s:\tARQOS 0x%X, AWQOS 0x%X, RMO 0x%.4X, WMO "\
					"0x%.4X, QUR(0x%X) TH_R 0x%.2X, TH_W 0x%.2X, ",
					info.name, stat.arqos, stat.awqos, stat.rmo, stat.wmo,
					stat.qurgent_on, stat.qurgent_th_r, stat.qurgent_th_w);
				if (stat.blocking_on)
					seq_printf(buf, "BLK(1) FR 0x%.4X, FW 0x%.4X, BR 0x%.4X, "\
						"BW 0x%.4X, MAX0_R 0x%.4X, MAX0_W 0x%.4X, "\
						"MAX1_R 0x%.4X, MAX1_W 0x%.4X\n",
						stat.qfull_limit_r, stat.qfull_limit_w,
						stat.qbusy_limit_r, stat.qbusy_limit_w,
						stat.qmax0_limit_r, stat.qmax0_limit_w,
						stat.qmax1_limit_r, stat.qmax1_limit_w);
				else
					seq_puts(buf, "BLK(0)\n");
			}
		}
...
static int exynos_bts_blocking_open_show(struct seq_file *buf, void *d)
{
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if (info[i].pd_on) {
			...
			if (stat.blocking_on)
				seq_printf(buf, "[%d] %s:   \tBLK(1) FR 0x%.4X, FW 0x%.4X, "\
					"BR 0x%.4X, BW 0x%.4X, MAX0_R 0x%.4X, MAX0_W 0x%.4X, "\
					"MAX1_R 0x%.4X, MAX1_W 0x%.4X\n", i, info[i].name,
					stat.qfull_limit_r, stat.qfull_limit_w,
					stat.qbusy_limit_r, stat.qbusy_limit_w,
					stat.qmax0_limit_r, stat.qmax0_limit_w,
					...
					size_t count, loff_t *ppos)
{
	...
	if (ret != 11) {
		pr_err("%s: sscanf failed. We need 11 inputs. <SCEN IP ON/OFF FULL_R, FULL_W, "\
			"BUSY_R, BUSY_W, MAX0_R, MAX0_W, MAX1_R, MAX1_W count=(%d)\n",
			__func__, ret);
		return -EINVAL;
	}
...
static int bts_parse_data(struct device_node *np, struct bts_device *data)
{
	...
	struct resource res;
	int i, j = 0;
	int ret = 0, check = 0, variant = 0;

	if (of_have_populated_dt()) {
		check = of_property_count_strings(np, "list-scen");
		if (check <= 0) {
		...
		for_each_child_of_node(np, child_np) {
			...
			if (of_property_read_u32(np, "variant", &variant)) {
				dev_warn(data->dev, "failed to get variant\n");
				ret = -EEXIST;
				goto err;
			}
			...
			case EXYNOSAUTO_V920_EVT2:
				ret = register_btsautov920_ops(&info[i]);
				break;
			case EXYNOSAUTO_V9:
				ret = register_btsautov9_ops(&info[i]);
				break;
			case EXYNOSAUTO_V620_EVT0:
				ret = register_btsautov620_ops(&info[i]);
				break;
			default:
				ret = -EEXIST;
				break;
			...
			if (ret) {
				dev_warn(data->dev, "failed to register ops\n");
				goto err;
			}
...
#if IS_ENABLED(CONFIG_DEBUG_FS) || IS_ENABLED(CONFIG_DEBUG_FS_MODULE)
...
static const struct of_device_id exynos_bts_match[] = {
	{
		.compatible = "samsung,exynosauto-bts",
	},
	{},
};
