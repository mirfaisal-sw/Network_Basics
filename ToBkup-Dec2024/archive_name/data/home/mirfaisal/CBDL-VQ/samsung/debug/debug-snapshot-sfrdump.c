...
static void dbg_snapshot_dump_sfr(void)
{
	...
	item = dbg_snapshot_get_item("log_sfr");
	if(!item)
		return;
	vaddr = item->entry.vaddr;
	if (!vaddr) {
	...
	list_for_each(entry, &sfrdump_list) {
		...
		for (i = 0; i < (sfrdump->size >> 2); i++) {
			...
			if (offset == 0 || (size_t)(sfrdump->reg + offset)/4 % 4 == 0)
				ret = snprintf(buf, SZ_64, "0x%08X: 0x%08X  ",
						sfrdump->phy_reg + offset, val);
			else
				ret = snprintf(buf, SZ_64, "0x%08X  ", val);


			if (ret < 0) {
				dev_err(sfrdump_dev, "snprintf is failed\n");
				break;
			}
			size = (int)strlen(buf);
			...
			memcpy((void *)(vaddr + dump_size), buf, size);
			dump_size += size;

			if ((size_t)(sfrdump->reg + offset)/4 % 4 == 3 || i ==
				(sfrdump->size >> 2) - 1) {
				ret = snprintf(buf, SZ_64, "\n");
				if (i == (sfrdump->size >> 2) - 1)
					ret = snprintf(buf, SZ_64, "\n\n\n");
