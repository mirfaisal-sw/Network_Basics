...
static void __touch_hardlockup_watchdog(void)
{
	struct hardlockup_watchdog_pcpu *pcpu_val =
				raw_cpu_ptr(hardlockup_watchdog_pcpu);
...
static bool is_hardlockup_other_cpu(unsigned int cpu)
{
	struct hardlockup_watchdog_pcpu *pcpu_val =
	...
	if (pcpu_val->hrtimer_interrupts_saved == hrint) {
		...
		if (time_after(now, touch_ts) &&
			...
			pr_crit("hardlockup-watchdog: cpu%x: hrint:%lu ",
				cpu, hrint);
			pr_crit("thresh:%lu, now:%lu, touch_ts:%lu\n",
				hardlockup_watchdog.thresh, now, touch_ts);
...
static int hardlockup_watchdog_enable(unsigned int cpu)
{
	struct hardlockup_watchdog_pcpu *pcpu_val =
				per_cpu_ptr(hardlockup_watchdog_pcpu, cpu);
	struct hrtimer *hrtimer;
...
static int hardlockup_watchdog_disable(unsigned int cpu)
{
	struct hardlockup_watchdog_pcpu *pcpu_val =
				per_cpu_ptr(hardlockup_watchdog_pcpu, cpu);
	struct hrtimer *hrtimer;
...
static int hardlockup_stop_fn(void *data)
{
	...
	if (hardlockup_watchdog_disable(cpu) != 0) {
		pr_err("%s: cpu:%x failed to disable\n", __func__, cpu);
		return -EAGAIN;
	}
	return 0;
}
...
{
	unsigned int cpu = raw_smp_processor_id();
	if (hardlockup_watchdog_enable(cpu) != 0) {
		pr_err("%s: cpu:%x failed to enable\n", __func__, cpu);
		return -EAGAIN;
	}
	return 0;
}
...
static int hardlockup_watchdog_online_cpu(unsigned int cpu)
{
	...
	if (cpu != raw_smp_processor_id() && !in_interrupt())
		smp_call_on_cpu(cpu, hardlockup_start_fn, NULL, false);
	else {
		if (hardlockup_watchdog_enable(cpu) != 0) {
			pr_err("%s: cpu:%x failed to enable\n", __func__, cpu);
...
static int hardlockup_watchdog_setup(void)
{
	int err;
...
static int hardlockup_watchdog_hrtimer_cancel_with_retry(
		struct hrtimer *hrtimer, u32 retry)
{
	int ret = -1;
...
static int hardlockup_watchdog_panic_handler(struct notifier_block *np,
				unsigned long l, void *msg)
{
	...
	pr_info("%s: disable hardlockup-watchdog\n", __func__);
	hardlockup_watchdog.enabled = false;

	for_each_cpu(cpu, &hardlockup_watchdog.allowed_mask) {
		if (cpu > num_possible_cpus())
			break;

		pr_info("%s: stop hrtimer for cpu%d\n", __func__, cpu);
		pcpu_val = per_cpu_ptr(hardlockup_watchdog_pcpu, cpu);
		hrtimer = &pcpu_val->hrtimer;

		cpumask_clear_cpu(cpu, &hardlockup_watchdog.allowed_mask);

		ret = hardlockup_watchdog_hrtimer_cancel_with_retry(hrtimer, 3);
		if (ret < 0)
			pr_err("Failed to stop hrtimer for cpu%d\n", cpu);
	}

	return 0;
}

static struct notifier_block hardlockup_watchdog_panic_nb = {
	.notifier_call = hardlockup_watchdog_panic_handler,
};
...
static int hardlockup_watchdog_probe(struct platform_device *pdev)
{
	...
	if (hardlockup_watchdog_setup() != 0) {
		pr_err("Hardlockup Watchdog setup failed \n");
		return -EAGAIN;
	}

	atomic_notifier_chain_register(&panic_notifier_list,
			&hardlockup_watchdog_panic_nb);
