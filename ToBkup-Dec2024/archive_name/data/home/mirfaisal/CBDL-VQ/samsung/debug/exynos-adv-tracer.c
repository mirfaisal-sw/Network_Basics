 ...
 *
 * EXYNOS - Advanced Tracer support
 */
...
static unsigned int pmu_dbgcore_config;
static unsigned int pmu_dbgcore_status;

static unsigned int shared_regs_offset = DEFAULT_MAILBOX_OFFSET;

static unsigned int SR(int n) { return (shared_regs_offset + (n << 2)); }
...
static irqreturn_t adv_tracer_ipc_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < adv_tracer_ipc->num_channels; i++) {
		...
		if (channel->used && status & (0x1 << channel->id)) {
			cmd = channel->cmd;
...
int adv_tracer_ipc_release_channel(unsigned int id)
{
	...
	adv_tracer_ipc->channel[id].used = false;
...
static void adv_tracer_ipc_channel_clear(void)
{
	...
	for (id = 1; id < EAT_MAX_CHANNEL; id++) {
		channel = &adv_tracer_ipc->channel[id];
		channel->used = false;
	}
}
...
static int adv_tracer_ipc_init(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "shared_regs_offset",
			&shared_regs_offset);

	if (ret) {
		dev_err(&pdev->dev, "failed to get shared register offset from dt : %d\n", ret);
		dev_err(&pdev->dev, "Shirinking to default value 0x%x\n", DEFAULT_MAILBOX_OFFSET);
		shared_regs_offset = DEFAULT_MAILBOX_OFFSET;
	}
