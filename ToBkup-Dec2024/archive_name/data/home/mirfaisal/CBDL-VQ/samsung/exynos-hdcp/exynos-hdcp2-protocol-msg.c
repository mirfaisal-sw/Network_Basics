...
int ske_generate_sessionkey(uint32_t node, uint32_t lk_type, uint8_t *enc_skey, int share_skey)
{
	int ret;

	ret = teei_generate_skey(node, lk_type,
...
int ske_generate_riv(uint32_t node, uint8_t *out, uint8_t *type)
{
	int ret;

	ret = teei_generate_riv(node, out, type, HDCP_RTX_BYTE_LEN);
	if (ret) {
...
int lc_generate_rn(uint32_t node, uint8_t *out, size_t out_len)
{
	int ret;

	ret = teei_gen_rn(node, out, out_len);
	if (ret) {
...
int lc_compare_hmac(uint32_t node, uint8_t *rx_hmac, size_t hmac_len)
{
	int ret;

	ret = teei_compare_lc_hmac(node, rx_hmac, hmac_len);
	if (ret) {
...
int ake_verify_cert(uint32_t node, uint8_t *cert, size_t cert_len,
		uint8_t *rrx, size_t rrx_len,
		uint8_t *rx_caps, size_t rx_caps_len)
{
	int ret;

	ret = teei_verify_cert(node, cert, cert_len,
			rrx, rrx_len,
			rx_caps, rx_caps_len);
	if (ret) {
...
int ake_generate_masterkey(uint32_t node, uint32_t lk_type, uint8_t *enc_mkey, size_t elen)
{
	int ret;

	/* Generate Encrypted & Wrapped Master Key */
	ret = teei_generate_master_key(node, lk_type, enc_mkey, elen);
	if (ret) {
...
int ake_compare_hmac(uint32_t node, uint8_t *rx_hmac, size_t rx_hmac_len)
{
	int ret;

	ret = teei_compare_ake_hmac(node, rx_hmac, rx_hmac_len);
	if (ret) {
...
int ake_store_master_key(uint32_t node, uint8_t *ekh_mkey, size_t ekh_mkey_len)
{
	int ret;

	ret = teei_set_pairing_info(node, ekh_mkey, ekh_mkey_len);
	if (ret) {
...
int ake_find_masterkey(uint32_t node, int *found_km,
		uint8_t *ekh_mkey, size_t ekh_mkey_len,
		uint8_t *m, size_t m_len)
{
	int ret;

	ret = teei_get_pairing_info(node, ekh_mkey, ekh_mkey_len, m, m_len);
	if (ret) {
