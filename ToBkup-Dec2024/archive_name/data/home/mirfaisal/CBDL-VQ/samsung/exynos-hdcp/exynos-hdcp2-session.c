...
enum hdcp_result hdcp_session_open(struct hdcp_sess_info *ss_info)
{
	...
	/* do open session */
	new_ss_node = (struct hdcp_session_node *) \
		kzalloc(sizeof(struct hdcp_session_node), GFP_KERNEL);

	if (!new_ss_node) {
		return HDCP_ERROR_INVALID_HANDLE;
	}

	new_ss = hdcp_session_data_create(ss_info->ss_id);
	if (!new_ss) {
		kfree(new_ss_node);
		return HDCP_ERROR_INVALID_HANDLE;
	}
...
enum hdcp_result hdcp_link_open(struct hdcp_link_info *link_info, uint32_t lk_type)
{
	...
	do {
		...
		new_lk_node = (struct hdcp_link_node *) \
				 kzalloc(sizeof(struct hdcp_link_node), GFP_KERNEL);
		if (!new_lk_node) {
			ret = HDCP_ERROR_MALLOC_FAILED;
			break;
		}
		new_lk_data = hdcp_link_data_create(ss_handle);
		if (new_lk_data == NULL) {
			ret = HDCP_ERROR_MALLOC_FAILED;
			break;
		}
...
struct hdcp_session_data *hdcp_session_data_create(uint32_t ss_id)
{
	struct hdcp_session_data *new_ss_data = NULL;
//	static int count; /* TODO change session id creation method */
//	count = 0;

	new_ss_data = (struct hdcp_session_data *) \
			kzalloc(sizeof(struct hdcp_session_data), GFP_KERNEL);
 ...
 */
struct hdcp_link_data *hdcp_link_data_create(uint32_t id)
{
	struct hdcp_link_data *new_lk_data = NULL;
//	static int count; /* TODO: change link id creation method */
//	count = 0;

	new_lk_data = (struct hdcp_link_data *) \
		kzalloc(sizeof(struct hdcp_link_data), GFP_KERNEL);
	if (!new_lk_data)
		return NULL;
