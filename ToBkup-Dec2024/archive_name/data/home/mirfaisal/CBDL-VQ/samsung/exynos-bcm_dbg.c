...
int exynos_bcm_dbg_pd_sync(unsigned int cal_pdid, bool on)
{
	...
	if (!bcm_dbg_data || !bcm_dbg_data->pd_sync_init) {
		BCM_DBG("%s: do not pd_sync_init(%s)\n",
			__func__, bcm_dbg_data->pd_sync_init ? "true" : "false");
		return 0;
	}
...
static int exynos_bcm_dbg_early_init(struct exynos_bcm_dbg_data *data)
{
	...
	for (ip_cnt = 0; ip_cnt < data->bcm_ip_nr; ip_cnt++) {
		...
		if (ret) {
			BCM_ERR("%s: failed set IP control (%d)\n", __func__, ip_cnt);
			return ret;
		}
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (ip_cnt = 0; ip_cnt < data->bcm_ip_nr; ip_cnt++) {
		...
		count += sysfs_emit_at(buf, count, "bcm[%2d]:", ip_cnt);
		for (ev_cnt = 0; ev_cnt < BCM_EVT_EVENT_MAX; ev_cnt++)
			count += sysfs_emit_at(buf, count,
					" ev%d %u,", ev_cnt,
					filter_id.sm_id_active[ev_cnt]);
...
static ssize_t show_enable_dump_file(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = container_of(dev,
					struct platform_device, dev);
...
static ssize_t store_enable_dump_file(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct platform_device *pdev = container_of(dev,
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < bcm_bw->num_ip; i++)
		count += snprintf(buf + count, PAGE_SIZE, "%llx,%x,%llx,%llx,%llx,%llx\n",
...
static ssize_t store_bcm_bw(struct device *dev,
				...
				const char *buf, size_t count)
{
	unsigned int enable, measure_time, num_ip, start_ip_idx, end_ip_idx;
	int i, ret, sample_cnt;

	ret = sscanf(buf, "%u %u %u %u", &enable, &measure_time, &start_ip_idx, &end_ip_idx);

	if (ret < 4) {
		BCM_ERR("sscanf is failed in store_bcm_bw\n");
		return count;
	}

	if (end_ip_idx < start_ip_idx) {
		BCM_ERR("end ip index is smaller than start ip index\n");
		return count;
	}

	num_ip = end_ip_idx - start_ip_idx + 1;
	if (num_ip > BCM_BW_NUM_IP_MAX) {
		BCM_ERR("requested data size for %u IPs exceed PAGE_SIZE\n", num_ip);
		return count;
	}
	...
	if (sample_cnt < measure_time) {
		BCM_ERR("Failed to set: sample_cnt:%d, measure_time: %d\n",
	...
	if (enable) {
		...
		for (i = 0; i < num_ip; i++)
			bcm_bw->ip_idx[i] = start_ip_idx + i;
...
static ssize_t bcm_dbg_dump_show(struct file *file, char __user *ubuf, size_t size, loff_t *ppos)
{
...
#endif
#if IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_DUMP_FILE)
static DEVICE_ATTR(enable_dump_file, 0640, show_enable_dump_file, store_enable_dump_file);
#endif
...
#endif
#if IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_DUMP_FILE)
	&dev_attr_enable_dump_file.attr,
#endif
...
static int exynos_bcm_dbg_init(void)
{
	return platform_driver_register(&exynos_bcm_dbg_driver);
}
late_initcall(exynos_bcm_dbg_init);
