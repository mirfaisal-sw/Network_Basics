...
{ \
	printf("Error: %s\n", info);\
	printf("Usage: write_devmem FILENAME ADDRESS(in hex format like 0x0100) [WIDTH [write value]]\n");\
	exit(1);\
}
...
int main(int argc, char **argv)
{
	...
	/*File Name*/
	if (!argv[1])
		show_usage("No filename found\n")

	if (1 != sscanf(argv[1], "%ms", &filename))
		show_usage("can't parse filename")

	memfd = open(filename, O_RDWR | O_SYNC);
	...
	if (1 != sscanf(argv[2], "0x%lx", &dev_base))
	...
	if (argv[3]) {
		if (isdigit(argv[3][0]) || argv[3][1]) {
			//width = xatou(argv[2]);
			width_ip = strtoul(argv[3], NULL, 10);
			if (width_ip < 0) {
		...
		/* VALUE */
		if (argv[4])
			writeval_ip = strtoull(argv[4], NULL, 0);
			if (writeval_ip < 0)
				show_usage("Invalid write value. Input a valid decimal write-value")
			writeval = writeval_ip;
	}
	...
	mapped_base = mmap(0, length, argv[4] ? (PROT_READ | PROT_WRITE) : PROT_READ,
				MAP_FIXED | MAP_SHARED, memfd,
				dev_base & ~MAP_MASK);
